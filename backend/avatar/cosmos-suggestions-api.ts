...
// Topic ideas are intentionally generated by a fast model. A child should see
// a usable idea almost immediately, not wait for a full writing-model request.
const SUGGESTION_MODEL = "google/gemini-3.1-flash-lite";
const CACHE_TTL_MS = 12 * 60 * 60 * 1000;
const MAX_LIST_CHARS = 1500;
const MAX_SUGGESTIONS_IN_CACHE = 56;
const INITIAL_SUGGESTION_COUNT = 12;
const INITIAL_SUGGESTION_TIMEOUT_MS = 10_000;
const REFRESH_SUGGESTION_TIMEOUT_MS = 5_000;
...
async function callSuggestionLlm(params: {
  ...
  const timeoutMs = params.timeoutMs ?? INITIAL_SUGGESTION_TIMEOUT_MS;
  const controller =
...
function buildInitialPrompt(params: {
  ...
  return [
    ...
    "- teaserKid: a vivid child-facing hook, max 14 words.",
    "- reasonParent: a concrete learning benefit, max 16 words.",
    params.strict
...
async function generateInitialSuggestions(params: {
  ...
  try {
    ...
    console.warn("[suggestions] initial Gemini generation failed, using fallback", error);
  }
...
async function generateOneSuggestion(params: {
  ...
  try {
    ...
    if (candidate) {
      const key = topicDedupKey(candidate.topicTitle, candidate.topicSlug);
      if (!excludeKeys.has(key)) return candidate;
    }
  } catch (error) {
    console.warn("[suggestions] refresh-one Gemini generation failed, using fallback", error);
  }
