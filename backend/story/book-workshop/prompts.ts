...
castIds enthält ausschließlich gewählte Nebenfiguren aus candidates. Die Hauptfiguren aus heroes sind bereits gesetzt und gehören NICHT in castIds. Ohne candidates ist castIds immer [].
...
Halte die Planung knapp. Pro beat je ein kurzer Satz für action/cause/result; cause benennt ein vorheriges Ereignis, nicht bloß ein Bindewort. Noch keine ausformulierte Prosa.`,
    user: JSON.stringify({
      ...
      output: {
        ...
        worldRule: "Eine Regel oder null bei einer alltäglichen Geschichte", castIds: [], artifactId: null,
...
export function manuscriptPrompt(brief: BookBrief, plan: BookPlan, repair?: { book: Manuscript; issues: string[] }) {
  const budget = readingBudget(brief);
  const targetWords = Math.round((budget.minWords + budget.maxWords) / 2);
  const wordsPerPage = Math.round(targetWords / budget.pages);
...
LÄNGE: Genau ${budget.pages} Leseseiten. Für ALLE Seiten ZUSAMMEN ${budget.minWords}-${budget.maxWords} Wörter Vorlesetext, Ziel ${targetWords}. Pro Seite im Mittel nur ${wordsPerPage} Wörter, nicht ${targetWords}! Plane dieses Wortbudget vor dem Schreiben ein. Verteile den Schluss auf die letzten Seiten, statt zusätzliche Absätze anzuhängen. Bildsätze zählen nicht zur Vorlese-Wortzahl.
...
Zusätzlich pro Seite genau EIN sichtbarer Moment als kurzer englischer Bildsatz mit höchstens 35 Wörtern und die IDs der darin sichtbaren Figuren (höchstens vier). Andere Figuren bleiben in der Erzählung und erhalten andere Bildmomente. Szene und Prosa müssen zusammenpassen. Keine Porträtrahmen, Referenztafeln, Collagen oder Texte im Bild. Nenne beim Bild die aktuelle Form und den Zustand wichtiger Gegenstände.
artifactVisible meint ausschließlich das ausgewählte Katalog-Artefakt, nicht gewöhnliche Requisiten. ${plan.artifactId ? "Setze es nur dann auf true, wenn dieses Artefakt im Bild sichtbar ist." : "Es ist kein Katalog-Artefakt ausgewählt: artifactVisible ist auf jeder Seite false."}`,
    user: JSON.stringify({
      ...
      output: { title: "Titel", description: "Ein kurzer vollständiger Satz", pages: Array.from({ length: budget.pages }, (_, i) => ({ order: i + 1, text: `Etwa ${wordsPerPage} Wörter Vorlesetext`, illustration: { scene: "English visible moment, at most 35 words", castIds: ["ID"], artifactVisible: false } })) },
    }),
  };
}
...
Kopiere Belege exakt aus der angegebenen Textseite, bevorzugt 5-12 zusammenhängende Wörter. Keine geänderte Zeitform, keine Zusammenfassung, keine Auslassungszeichen. Die Software prüft jedes Zitat gegen diese Seite.
