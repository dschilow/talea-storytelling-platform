    ...
    try {
      ...
      try {
        for (const avatar of input.avatars) {
          const rows: Array<{ id: string; story_title: string; emotional_impact: string; summary: string | null }> = [];
          const gen = await avatarDB.query<{ id: string; story_title: string; emotional_impact: string; summary: string | null }>`
            WITH ranked AS (
              SELECT
                id,
                story_title,
