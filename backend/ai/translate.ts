...
export async function translateToEnglish(text: string): Promise<string> {
  ...
  const germanWords = /\b(ist|und|oder|der|die|das|ein|eine|aus|hat|sind|mit|von|für|auf|zu|im|am|über|bei|durch|nach|vor|zwischen|unter|während|seit|bis|gegen|ohne|als|wenn|dass|weil|aber|doch|jedoch|nicht|sehr|groß|klein|jung|schön|hässlich|schnell|langsam|schwach|hell|dunkel|kalt|Jahr|Jahre|Monat|Monate|Mensch|Menschen|Kinder|Junge|Mädchen|Mann|Frau|Tier|Tiere|Hund|Katze|Vogel|Fisch|Baum|Blume|Haus|Stadt|Wasser|Feuer|Himmel|Sonne|Mond|Stern|Sterne|Farbe|Farben|blau|grün|gelb|lila|schwarz|weiß|grau|braun|Auge|Augen|Ohren|Nase|Mund|Hände|Füße|Kopf|Haar|Haare|Gesicht|Körper|Seele|Geist)\b/gi;
...
Output: "Diego is an orange cat with white belly"`,
    ...
    if (!response.ok) {
      console.error("[translateToEnglish] OpenAI API error", { status: response.status });
      return text;
    }
    ...
    if (!translatedText) {
      console.error("[translateToEnglish] OpenAI returned no translation");
      return text;
    }

    return translatedText;
  } catch (error: any) {
    console.error("[translateToEnglish] Translation request failed", {
