...
const SQL_FILE = join(scriptDir, "backend", "story", "migrations", "35_disable_soul_stage_emergency.up.sql");
...
async function main(): Promise<void> {
  console.log("Talea emergency Soul-stage disable migration");
  ...
  await postSql(sql, "35_disable_soul_stage_emergency");
  await postSql(VERIFY_SQL.trim(), "verify_35_disable_soul_stage_emergency");

  console.log("\nSUCCESS: Soul stage is disabled and verified.");
}

main().catch((error) => {
  console.error("\nFAILED: emergency Soul-stage disable migration failed.");
