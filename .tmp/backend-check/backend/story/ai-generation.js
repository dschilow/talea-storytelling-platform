...
function clampRunwarePrompt(prompt, maxLength = 2800) {
  if (!prompt) {
    return "";
  }
  ...
  if (charactersIndex !== -1) {
    ...
    const charactersEndIndex = nextSectionMatch ? charactersIndex + (nextSectionMatch.index || afterCharacters.length) : charactersIndex + Math.min(800, afterCharacters.length);
    ...
    if (availableForRest > 100) {
      ...
      const lastGoodBreak = Math.max(truncatedRest.lastIndexOf("."), truncatedRest.lastIndexOf(`
`), truncatedRest.lastIndexOf(","));
      const cleanRest = lastGoodBreak > availableForRest * 0.5 ? truncatedRest.substring(0, lastGoodBreak + 1) : truncatedRest;
  ...
  const lastSeparator = Math.max(sliced.lastIndexOf("."), sliced.lastIndexOf(","), sliced.lastIndexOf(";"), sliced.lastIndexOf(" "));
  const cleanSlice = lastSeparator > maxLength * 0.6 ? sliced.slice(0, lastSeparator) : sliced;
  return `${cleanSlice.trimEnd()}...`;
}
var INLINE_TTS_TAG_PATTERN = /\[([^\]\n]{1,40})\]/g;
var KNOWN_TTS_TAGS = new Set([
  "excited",
  "dramatic",
  "thoughtful",
  "curious",
...
function isLikelyTtsEmotionTag(tag) {
  const normalized = normalizeTag(tag);
  if (!normalized)
    return false;
  if (KNOWN_TTS_TAGS.has(normalized))
    return true;
  if (normalized.includes("pause") || normalized.includes("beat"))
    return true;
  if (normalized.includes("whisper"))
    return true;
  if (normalized.includes("laugh"))
    return true;
  if (normalized.includes("excit"))
    return true;
  if (normalized.includes("dramatic"))
    return true;
  return false;
}
function stripTtsEmotionTagsForImagePrompt(text) {
  const raw = String(text || "");
  if (!raw)
    return "";
  return raw.replace(INLINE_TTS_TAG_PATTERN, (fullTag, innerTag) => isLikelyTtsEmotionTag(String(innerTag || "")) ? " " : fullTag).replace(/[ \t]+/g, " ").replace(/\s+([,.;!?])/g, "$1").trim();
}
var STYLE_PRESET_META = {
  rhymed_playful: {
    inspiration: "Inspired by 'Der Gr\xFCffelo': rhythmic, playful cadence with call-and-response dialogue; keep lines musical. Always dialogue-driven with action.",
    description: "Rhymed phrases, call-and-response, humorous dialogue."
  },
  gentle_minimal: {
    inspiration: "Inspired by 'Die kleine Raupe Nimmersatt': minimal structure with repeating phrases; calm but always action-based. Characters DO things, not just exist.",
    description: "Repetition, clear structure, calm actions."
  },
  wild_imaginative: {
    inspiration: "Inspired by 'Wo die wilden Kerle wohnen': bold energy with safe boundaries; celebrate curiosity through concrete physical action and dialogue.",
    description: "Rebellious imagination, safe boundaries, action-driven."
  },
  philosophical_warm: {
    inspiration: "Inspired by 'Der kleine Prinz': warm, reflective tone with ONE simple wisdom per chapter \u2014 delivered through dialogue, not narration. No poetic metaphors.",
    description: "Small wisdoms through dialogue, grounded."
  },
  mischief_empowering: {
    inspiration: "Inspired by 'Pippi Langstrumpf': mischievous, empowering tone where kids act confidently and humor drives the plot through dialogue and slapstick.",
    description: "Self-efficacy, humor, dialogue-heavy."
  },
  adventure_epic: {
    inspiration: "Inspired by 'Harry Potter': episodic adventure with clear quests and team spirit; told through character dialogue and action, not description.",
    description: "Quest feeling, dialogue-driven action."
  },
  quirky_dark_sweet: {
    inspiration: "Inspired by 'Charlie und die Schokoladenfabrik': quirky, gently dark sweetness with surprising twists \u2014 shown through situational comedy and dialogue.",
    description: "Slightly quirky, always friendly, situational humor."
  },
  cozy_friendly: {
    inspiration: "Inspired by 'Winnie Puuh': cozy, dialogue-rich scenes full of friendship and gentle warmth. At least 50% dialogue.",
    description: "Cozy dialogue, friendship, character-driven."
  },
  classic_fantasy: {
    inspiration: "Inspired by 'Peter Pan': timeless fairy-tale fantasy with wide-eyed heroes; told through action sequences and character dialogue, not atmospheric description.",
    description: "Timeless fantasy, action-driven."
  },
  whimsical_logic: {
    inspiration: "Inspired by 'Alice im Wunderland': playful logic puzzles and wordplay delivered through character dialogue; easy to follow for kids.",
    description: "Logic games, playful dialogue."
  },
  mythic_allegory: {
    inspiration: "Inspired by 'Die Chroniken von Narnia': mythic storytelling with team spirit; symbolic moments shown through action and character choices, not poetic narration.",
    description: "Symbolism through action, team spirit."
  },
  road_fantasy: {
    inspiration: "Inspired by 'Der Zauberer von Oz': journey fantasy with clear stages and memorable companions; each stage shown through dialogue and concrete encounters.",
    description: "Journey, stages, companions, encounter-driven."
  },
  imaginative_meta: {
    inspiration: "Inspired by 'Die unendliche Geschichte': meta-fantasy celebrating imagination; stories inside stories told through character interaction, not atmospheric prose.",
    description: "Story within story, character-driven."
  },
  pastoral_heart: {
    inspiration: "Inspired by 'Heidi': heartfelt community and gentle resilience; warmth shown through character relationships and dialogue, not nature descriptions.",
    description: "Community feeling, dialogue-driven warmth."
  },
  bedtime_soothing: {
    inspiration: "Inspired by 'Gute Nacht, Mond': soothing bedtime tone with calm, short sentences and gentle rhythm. Still needs character action \u2014 just slower and softer.",
...
function resolveModelConfig(config) {
  if (config.aiProvider === "openrouter") {
    const modelName = normalizeOpenRouterModel(config.openRouterModel);
...
async function getAvatarProfilesFromDB(avatarIds) {
  const profiles = [];
...
function parseStoredTraitChanges(value) {
  try {
...
async function getAvatarMemoriesFromDB(avatarId, limit = 2) {
...
async function getAvatarProgressStatsFromDB(avatarId) {
  const row = await avatarDB.queryRow`
    SELECT
...
function describeCharacter(group, label) {
  if (!Array.isArray(group) || group.length === 0) {
    return [];
  }
  return group.map((entry) => {
    const parts = [label + ": " + stripTtsEmotionTagsForImagePrompt(entry.name)];
    if (entry.action)
      parts.push(stripTtsEmotionTagsForImagePrompt(entry.action));
    if (entry.position)
      parts.push("position " + stripTtsEmotionTagsForImagePrompt(entry.position));
    if (entry.expression)
      parts.push("expression " + stripTtsEmotionTagsForImagePrompt(entry.expression));
    if (entry.pose)
      parts.push("pose " + stripTtsEmotionTagsForImagePrompt(entry.pose));
    if (entry.clothingHint)
      parts.push("clothing " + stripTtsEmotionTagsForImagePrompt(entry.clothingHint));
    if (entry.ageGuard)
      parts.push(stripTtsEmotionTagsForImagePrompt(entry.ageGuard));
    return parts.join(", ");
  });
}
function summarizeAtmosphere(atmosphere) {
  if (!atmosphere)
    return [];
  const lines = [];
  if (atmosphere.weather)
    lines.push("Weather: " + stripTtsEmotionTagsForImagePrompt(atmosphere.weather));
  if (atmosphere.lighting)
    lines.push("Lighting: " + stripTtsEmotionTagsForImagePrompt(atmosphere.lighting));
  if (atmosphere.season)
    lines.push("Season: " + stripTtsEmotionTagsForImagePrompt(atmosphere.season));
  if (atmosphere.mood)
    lines.push("Mood: " + stripTtsEmotionTagsForImagePrompt(atmosphere.mood));
  if (Array.isArray(atmosphere.sensoryDetails) && atmosphere.sensoryDetails.length > 0) {
    lines.push("Sensory: " + atmosphere.sensoryDetails.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  return lines;
}
function summarizeEnvironment(layers) {
  if (!layers)
    return [];
  const lines = [];
  if (Array.isArray(layers.foreground) && layers.foreground.length > 0) {
    lines.push("Foreground: " + layers.foreground.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  if (Array.isArray(layers.midground) && layers.midground.length > 0) {
    lines.push("Midground: " + layers.midground.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  if (Array.isArray(layers.background) && layers.background.length > 0) {
    lines.push("Background: " + layers.background.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  return lines;
}
function extractImagePromptContext(description) {
  if (!description) {
    return {
      summary: "",
      protagonistLines: [],
  ...
  if (Array.isArray(description.props) && description.props.length > 0) {
    narrativePieces.push("Props: " + description.props.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  ...
  if (Array.isArray(description.storytellingDetails) && description.storytellingDetails.length > 0) {
    narrativePieces.push("Story notes: " + description.storytellingDetails.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).join(", "));
  }
  const summary = narrativePieces.join(" ");
  const limitedEnvironment = {
    foreground: Array.isArray(description.environment?.foreground) ? description.environment.foreground.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).slice(0, 4) : [],
    midground: Array.isArray(description.environment?.midground) ? description.environment.midground.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).slice(0, 6) : [],
    background: Array.isArray(description.environment?.background) ? description.environment.background.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).slice(0, 4) : []
  };
  const limitedProps = Array.isArray(description.props) ? description.props.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).slice(0, 6) : [];
  const limitedStoryDetails = Array.isArray(description.storytellingDetails) ? description.storytellingDetails.map((item) => stripTtsEmotionTagsForImagePrompt(item)).filter(Boolean).slice(0, 5) : [];
  return {
    summary,
    protagonistLines,
    supportingLines,
    environmentLayers: limitedEnvironment.foreground.length || limitedEnvironment.midground.length || limitedEnvironment.background.length ? limitedEnvironment : undefined,
    props: limitedProps,
    atmosphereLines,
    recurringElementNote: description.recurringElement ? stripTtsEmotionTagsForImagePrompt(description.recurringElement) : undefined,
    storytellingDetails: limitedStoryDetails
  };
}
function normalizeRunwareDimensions(width, height) {
  const roundToMultiple64 = (value) => Math.round(value / 64) * 64;
  return {
    width: Math.max(128, Math.min(2048, roundToMultiple64(width))),
    height: Math.max(128, Math.min(2048, roundToMultiple64(height)))
  };
}
var generateStoryContent = api({ expose: false, method: "POST", path: "/ai/generate-story" }, async (req) => {
  ...
  try {
    ...
    const avatarIdMappings = req.avatarDetails.map((a) => ({
      id: a.id,
      name: a.name
    }));
    ...
    const storyOutcome = await generateEnhancedStoryWithOpenAI(req.config, req.avatarDetails, QUALITY_CONFIG.MAX_RETRIES);
    ...
    metadata.totalCost.text = metadata.tokensUsed.prompt / 1e6 * selectedModel.inputCostPer1M + outputTokens / 1e6 * selectedModel.outputCostPer1M;
    console.log(`[ai-generation] \uD83D\uDD0D Story chapters before cleanup:`, {
      chapterCount: storyOutcome.story.chapters?.length ?? 0,
      chapters: storyOutcome.story.chapters?.map((ch, idx) => ({
        index: idx,
        hasTitle: !!ch?.title,
    ...
    if (storyOutcome.story.chapters) {
      ...
      storyOutcome.story.chapters = storyOutcome.story.chapters.filter((ch) => ch && (ch.title?.trim() || ch.content?.trim()));
      ...
      storyOutcome.story.chapters = storyOutcome.story.chapters.map((ch, idx) => ({
        ...ch,
        order: ch.order ?? idx
      }));
    }
    ...
    if (!Array.isArray(normalizedStory.supportingCharacters)) {
      normalizedStory.supportingCharacters = [];
    }
    if (!normalizedStory.recurringElement || typeof normalizedStory.recurringElement !== "object") {
    ...
    if (!Array.isArray(normalizedStory.plotBeats)) {
      normalizedStory.plotBeats = [];
    }
    const avatarProfilesByName = {};
    storyOutcome.state.avatarProfilesByName.forEach((profile, name) => {
      avatarProfilesByName[name] = profile;
    });
    const missingProfiles = req.avatarDetails.filter((av) => !avatarProfilesByName[av.name]);
    if (Object.keys(avatarProfilesByName).length === 0) {
      console.warn("[ai-generation] \u26A0\uFE0F Keine Avatarprofile \xFCber Tool-Aufrufe erhalten \u2013 Fallback auf direkten DB-Aufruf.");
      ...
      console.warn(`[ai-generation] ${missingProfiles.length} Avatare ohne visualProfile erkannt:`, missingProfiles.map((a) => a.name));
      missingProfiles.forEach((avatar) => {
        console.log(`[ai-generation] Erstelle Fallback-Profil f\xFCr Avatar "${avatar.name}"`);
        const fallbackProfile = createFallbackProfile(avatar);
        avatarProfilesByName[avatar.name] = fallbackProfile;
        console.log(`[ai-generation] \u2705 Fallback-Profil f\xFCr "${avatar.name}" erstellt (v${fallbackProfile.version}, hash: ${fallbackProfile.hash.substring(0, 8)})`);
      });
    }
    console.log(`[ai-generation] Avatar profiles verf\xFCgbar:`, {
    ...
    const storyConsistency = createStoryConsistency(storyTitle, avatarDataForConsistency, req.config.ageGroup || "6-8", TALEA_DEFAULT_STYLE);
    ...
    const characterFirstBlock = buildCharacterFirstBlock(storyConsistency.characters, storyConsistency.lockedStyle);
    console.log(`[ai-generation] \uD83D\uDCCB Character-first block created (${characterFirstBlock.length} chars)`);
    const coverImageDescription = typeof normalizedStory.coverImageDescription === "object" ? normalizedStory.coverImageDescription : undefined;
    const firstChapterImageDescription = typeof normalizedStory.chapters?.[0]?.imageDescription === "object" ? normalizedStory.chapters?.[0]?.imageDescription : undefined;
    ...
    let safeCoverSceneText = safeCoverScene(coverContext.summary, firstChapterContext.summary);
...
${coverPrompts.positivePrompt}`;
    ...
    const coverPromptClamped = clampRunwarePrompt(coverPromptNormalized, 3200);
    ...
    for (let i = 0;i < normalizedStory.chapters.length; i++) {
      ...
      const chapterImageDescription = typeof chapter.imageDescription === "object" ? chapter.imageDescription : undefined;
      const chapterContext = extractImagePromptContext(chapterImageDescription);
      let chapterSceneText = typeof chapter.imageDescription === "string" ? stripTtsEmotionTagsForImagePrompt(chapter.imageDescription) : chapterContext.summary;
...
${chapterPrompts.positivePrompt}`;
      ...
      const chapterPromptClamped = clampRunwarePrompt(chapterPromptNormalized, 3200);
    ...
    const chaptersWithImages = normalizedStory.chapters.map((chapter, index) => ({
      ...chapter,
    ...
    const telemetry = createTelemetry({
      ...
      avatarIds,
      profileHashes: Object.fromEntries(Object.entries(versionedProfiles).map(([name, profile]) => [name, profile.hash])),
      profileVersions: Object.fromEntries(Object.entries(versionedProfiles).map(([name, profile]) => [name, profile.version])),
      positivePrompt: coverPromptNormalized.substring(0, 500),
      negativePrompt: "",
      seed: seedBase,
      cfg: 4,
      steps: 4,
...
var MAX_TOOL_MEMORIES = 2;
function summarizeTraitChanges(changes) {
...
function normalizeTraitChanges(changes) {
  ...
  return changes.map((entry) => {
    ...
    const trait = typeof entry.trait === "string" && entry.trait.trim().length > 0 ? entry.trait.trim() : "";
    const rawChange = typeof entry.change === "number" ? entry.change : typeof entry.change === "string" ? Number(entry.change) : NaN;
...
function enforceAvatarDevelopments(story, avatars) {
  if (!story || typeof story !== "object" || !Array.isArray(avatars)) {
    return;
  }
  const assignedById = new Map(assignAvatarDevelopmentIds(story.avatarDevelopments, avatars).map((development) => [development.avatarId, development]));
  const enforced = avatars.map((avatar) => assignedById.get(avatar.id) ?? {
    avatarId: avatar.id,
    name: avatar.name,
    changedTraits: []
  });
...
function validateGeneratedStory(story, avatars, config) {
  const issues = [];
  const suggestions = [];
  let score = 10;
  ...
  const fullText = chapters.map((c) => String(c?.content || c?.text || "")).join(" ");
  ...
  if (config?.ageGroup) {
    ...
    const range = QUALITY_CONFIG.AVG_SENTENCE_LENGTH[config.ageGroup];
...
function getCharacterVoice(traits) {
  if (!traits)
    return "spricht freundlich und neugierig";
  const getTraitValue = (t) => typeof t === "object" && t !== null ? t.value ?? 0 : t ?? 0;
  ...
  const voice = [];
  if (courage > 70)
    voice.push("spricht mutig und direkt");
  else if (courage < 30)
    voice.push("spricht vorsichtig und fragt oft nach");
  if (humor > 70)
    voice.push("macht Witze");
  else if (humor < 30)
    voice.push("ist ernst und nachdenklich");
  if (intelligence > 70)
    voice.push("benutzt manchmal schwierige Woerter");
...
function getSettingAtmosphere(setting) {
  const atmospheres = {
...
function getPersonalityUpdateInstructions() {
  return `
...
Wissen wird ausschlie\xDFlich durch Doku-Lekt\xFCre erworben, NICHT durch Geschichten.
...
function buildEnhancedUserPrompt(config, avatars, chapterCount, qualityFeedback) {
...
- Dichte: ca. 1-3 Marker pro 120 Woerter (nicht uebertreiben).`.trim();
  const characterProfiles = avatars.map((avatar, index) => {
    ...
    const topTraits = Object.entries(personalityTraits).sort(([, a], [, b]) => b - a).slice(0, 3).map(([trait, value]) => `${trait} (${value}/100)`).join(", ");
    return `
### ${avatar.name} [${role}]
...
async function generateEnhancedStoryWithOpenAI(config, avatars, maxRetries = QUALITY_CONFIG.MAX_RETRIES) {
  let attempt = 0;
  let bestResult = null;
  let bestScore = 0;
  let feedback;
...
async function generateStoryWithOpenAITools(args) {
  ...
  const stylePresetMeta = config.stylePreset && STYLE_PRESET_META[config.stylePreset] ? STYLE_PRESET_META[config.stylePreset] : undefined;
...
You MUST implement this style consistently in ALL chapters!` : "";
  ...
  const avatarVisualLines = avatars.map((avatar) => {
    ...
    const details = [];
    if (vp.characterType)
      details.push(String(vp.characterType));
    ...
    if (vp.eyes?.color)
      details.push(vp.eyes.color + " eyes");
    if (vp.skin?.tone)
      details.push(vp.skin.tone + " skin");
    ...
    if (vp.clothingCanonical?.top)
      details.push(vp.clothingCanonical.top);
    if (vp.clothingCanonical?.outfit)
      details.push(vp.clothingCanonical.outfit);
...
`);
  const avatarIntegrationExamples = avatars.map((avatar) => {
    const vp = avatar.visualProfile;
    const examples = [];
    if (vp?.hair?.color) {
...
`);
  const avatarSummary = avatars.map((avatar) => {
...
`);
  ...
  const avatarMemoriesMap = new Map;
  ...
  for (const avatarId of avatarIds) {
    ...
    console.log(`[ai-generation] Avatar ${avatarId}: ${memories.length} memories, stats S:${stats.storiesRead} D:${stats.dokusRead} M:${stats.memoryCount}`);
  }
  const compressedMemoriesMap = new Map;
  ...
  const avatarMemoryLines = avatars.map((avatar) => {
    ...
    if (!relevantMemory)
      return null;
    const traitLine = relevantMemory.dominantTraits.length > 0 ? ` | Traits: ${relevantMemory.dominantTraits.join(", ")}` : "";
    return `${avatar.name}: ${relevantMemory.title} -> ${relevantMemory.summary}${traitLine}`;
  }).filter(Boolean);
  const memorySection = avatarMemoryLines.length > 0 ? ["MEMORY CALLBACKS (reuse only if fitting):", ...avatarMemoryLines, ""].join(`
`) : "";
  const progressionLines = avatars.map((avatar) => {
    ...
    const focus = progression.traitMastery.find((trait) => trait.trait === progression.focusTrait);
    const topPerks = progression.perks.filter((perk) => perk.unlocked).slice(0, 2).map((perk) => perk.title);
    const activeQuest = progression.quests.find((quest) => quest.status === "active");
    const perkText = topPerks.length > 0 ? `Perks: ${topPerks.join(", ")}` : "Perks: none yet";
    const questText = activeQuest ? `Quest: ${activeQuest.title} ${activeQuest.progress}/${activeQuest.target}` : "Quest: all tracked quests completed";
    return `${avatar.name}: Focus ${focus?.label || "Wissen"} (${focus?.rank.name || "Anfaenger"}) | ${perkText} | ${questText}`;
  }).filter(Boolean);
  const progressionSection = progressionLines.length > 0 ? ["AVATAR GROWTH CONTEXT:", ...progressionLines, ""].join(`
`) : "";
  const userPrompt = [
...
`);
  ...
  if (isMiniMaxModel) {
    ...
    try {
      ...
      if (!content)
        throw new Error("Empty response from GPT fallback");
    ...
    const openRouterTimeoutMs = config.length === "long" ? 360000 : config.length === "medium" ? 240000 : 180000;
    ...
    const openAITimeoutMs = config.length === "long" ? 360000 : config.length === "medium" ? 240000 : 180000;
    const abortController = new AbortController;
  ...
  try {
    ...
    let tempParsed = JSON.parse(cleanContent);
  ...
  const inputCostUSD = usageTotals.prompt / 1e6 * modelConfig.inputCostPer1M;
  const outputCostUSD = usageTotals.completion / 1e6 * modelConfig.outputCostPer1M;
  const totalCostUSD = inputCostUSD + outputCostUSD;
  console.log(`[ai-generation] \uD83D\uDCB0 Cost breakdown:`);
  console.log(`  Input: ${usageTotals.prompt} tokens \xD7 $${modelConfig.inputCostPer1M}/1M = $${inputCostUSD.toFixed(4)}`);
  console.log(`  Output: ${usageTotals.completion} tokens \xD7 $${modelConfig.outputCostPer1M}/1M = $${outputCostUSD.toFixed(4)}`);
