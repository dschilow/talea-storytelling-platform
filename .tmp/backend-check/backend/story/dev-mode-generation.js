...
var DEV_MODE_IMAGE_MODEL = "runware:400@4";
...
var DEV_MODE_BROAD_DIALOG_REPAIR_PCT = 18;
var DEV_MODE_TARGET_DIALOG_PCT = 28;
var DEV_MODE_PROMPT_DIALOG_PCT = 32;
...
var DEV_MODE_MAX_VALIDATION_POLISH_ATTEMPTS = 1;
var DEV_MODE_DIMINISHING_RETURNS_SCORE = 8.5;
var ABSTRACT_COST_FEELING = "(?:zuversicht|scham|angst|mut|vertrauen|hoffnung|geduld|selbstvertrauen|sicherheit|stolz|freude|ruhe|wuerde|w\xFCrde|confidence|courage|hope|shame|fear|trust|pride|patience|dignity)";
var ABSTRACT_COST_ARTICLE = "(?:seine[nrs]?\\s+|ihre[nrs]?\\s+|den\\s+|die\\s+|das\\s+|the\\s+|his\\s+|her\\s+|their\\s+)?";
var ABSTRACT_COST_VERB = "(?:verlier\\w*|verlor\\w*|(?:ue|\xFC)berwind\\w*|gewinn\\w*|fass\\w*|find\\w*|loses?|lose|losing|gains?|overcome\\w*|regains?)";
var ABSTRACT_PERSONAL_COST_PATTERN = new RegExp(`(?:${ABSTRACT_COST_VERB}\\s+${ABSTRACT_COST_ARTICLE}${ABSTRACT_COST_FEELING}\\b)` + `|(?:${ABSTRACT_COST_FEELING}\\s+(?:\\w+\\s+){0,2}?(?:(?:ue|\xFC)berwind\\w*|verlier\\w*|verlor\\w*|fass\\w*|zu\\s+(?:ue|\xFC)berwinden))`, "i");
...
var ANCHOR_CONTAMINATION_MOTIFS = [
  "Gl\xF6ckchen",
  "Gloeckchen",
  "Ger\xE4usche-Fresser",
  "Geraeusche-Fresser",
  "lautlose Stadt",
  "m\xFCdes Kissen",
  "muedes Kissen",
  "gestohlene Ger\xE4usche",
...
function resolveDevModeChapterCount(input) {
  const override = Number(input.chapterCountOverride);
  if (Number.isFinite(override))
    return Math.max(3, Math.min(8, Math.trunc(override)));
...
function localizedLanguageName(language) {
  switch (language) {
    case "en":
      return "English";
    case "fr":
      return "French (fran\xE7ais)";
    case "es":
      return "Spanish (espa\xF1ol)";
    case "it":
...
var NORMALIZED_NOVELTY_STOPWORDS = new Set([...NOVELTY_STOPWORDS].map((word) => normalizeNoveltyText(word)));
function isNoveltyStopword(value) {
  return NORMALIZED_NOVELTY_STOPWORDS.has(normalizeNoveltyText(value));
}
function extractMotifKeywords(text, limit = 8) {
  const words = normalizeNoveltyText(text).split(/\s+/).map((word) => word.trim()).filter((word) => word.length >= 5 && !isNoveltyStopword(word));
...
var FORBIDDEN_MOTIF_LOCAL_REPAIRS = {
  ...
  toastgeraet: ["Fr\xFChst\xFCckskorb", "Fr\xFChst\xFCckstablett"],
  toaster: ["Fr\xFChst\xFCckskorb", "Fr\xFChst\xFCckstablett"],
...
function characterNameMotifAliases(name) {
  ...
  if (!normalized)
    return [];
  const parts = normalized.split(/\s+/).map((part) => part.trim()).filter((part) => part.length >= 4 && !isNoveltyStopword(part));
...
function buildNoveltyPromptBlock(input) {
  ...
  const trueHardAvoid = brief.hardAvoidMotifs.filter((motif) => !isNoveltyStopword(normalizeNoveltyText(motif))).filter(isHardBanMotif).slice(0, 18);
  const softAvoid = brief.hardAvoidMotifs.filter((motif) => !isNoveltyStopword(normalizeNoveltyText(motif))).filter((motif) => !isHardBanMotif(motif)).slice(0, 18);
  return [
...
function artifactNameCandidates(input) {
  ...
  const firstContentWords = raw.flatMap((name) => String(name).split(/\s+/)).map((word) => word.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, "").trim()).filter((word) => word.length >= 5 && !isNoveltyStopword(normalizeNoveltyText(word))).slice(0, 2);
...
function artifactNoveltyConflictMotifs(input) {
  ...
  for (const motif of input.noveltyBrief.hardAvoidMotifs) {
    ...
    if (normalizedMotif.length < 5 || isNoveltyStopword(normalizedMotif))
      continue;
...
function buildWonderRuleConsistencyBlock(input) {
  ...
  const hasColorCost = /farb|color|colour|blass|grau|bleich/.test(ruleLower) && /preis|kostet|kosten|opfer|verlier|verblasst|verschwind|schwindet|fades|vanish|loss/.test(ruleLower);
  const lines = [
...
async function generateDevModeImages(input, parsedTitle, parsedChapters, screenplayPlan) {
  ...
  const providerImageCostUSD = (result) => {
...
`);
  ...
  const looksLikeEnglishPrompt = (text) => looksLikeEnglishImagePrompt(text, castProperNouns);
  const sanitizeImagePrompt = (s) => {
    let out = String(s || "");
    const unwrapped = unwrapJsonPrompt(out);
    if (unwrapped.changed) {
      console.log("[dev-mode-generation] \xA712A unwrapped JSON envelope from image prompt", {
...
${ch.content.slice(0, 1200)}`
  ...
  for (const card of screenplayPlan?.sceneCards || []) {
    ...
    const safeExplicitNames = normalizedExplicitNames.filter((name) => proseKeys.has(normalizeCharacterName(name)));
  ...
  const fallbackImagePrompt = (job) => {
    ...
    const actionSource = `${scenePlan?.dramaticBeat || ""} ${scenePlan?.sceneSummary || ""}`.toLowerCase();
    const sceneActionHint = /riss|rei(?:ss|\u00DF)t|platz|zerrei/.test(actionSource) ? `the ${storyAnchor} tears open and releases a burst of light` : /schrumpf|kleiner|zwerghaft/.test(actionSource) ? `a threatening presence visibly shrinks as the ${storyAnchor} glows` : /wachs|gro(?:ss|\u00DF)er|riesig/.test(actionSource) ? `the ${storyAnchor} flares while the danger suddenly grows` : /stolper|fall|fiel|rutsch|purzel/.test(actionSource) ? `a sudden slip turns the search around ${storyAnchor} into urgent action` : /band|fessel|wickel|gefangen/.test(actionSource) ? `one character struggles free from a magical binding beside ${storyAnchor}` : /such|find|hinweis|entdeck/.test(actionSource) ? `the named cast discovers a decisive clue beside ${storyAnchor}` : /renn|flieh|jag|verfolg/.test(actionSource) ? `the named cast rushes through the setting to protect ${storyAnchor}` : englishVisualHint(actionSource, `one decisive action changes the situation around ${storyAnchor}`, 180);
  ...
  let imageResults = await mapWithConcurrency(jobs, 3, async (job) => {
    try {
      ...
      sanitizedPrompt = sanitizedPrompt.replace(/\b(?:human|humanoid|person-shaped)\s+(?:shadow|silhouette)\b/gi, "faceless non-humanoid shadow cloud").replace(/\bshadowy\s+(?:person|figure|child|boy|girl)\b/gi, "faceless non-humanoid shadow cloud").slice(0, 700);
...
`);
      ...
      if (!preflight.ok && preflight.issues.some((i) => i.code === "json_wrapper" || i.code === "missing_count_contract")) {
        console.warn(`[dev-mode-generation] \xA712 preflight FAILED, dropping image job`, {
          job: `${job.kind}${job.order ? `:ch${job.order}` : ""}`,
          issues: preflight.issues
        });
        return { job, imageUrl: undefined, fullPrompt, sceneRefs: [], providerCostUSD: 0 };
      }
      imageCalls += 1;
      ...
      return { job, imageUrl: undefined, fullPrompt: job.prompt, sceneRefs: [], providerCostUSD: 0 };
    }
  });
  const seenGeneratedImageUrls = new Set;
...
`);
      try {
        ...
        return { job, imageUrl: undefined, fullPrompt: retryPrompt, sceneRefs: [], providerCostUSD: 0 };
      }
    });
  ...
  if (visualQaEnabled && imagesGenerated > 0) {
    const allQaCandidates = imageResults.filter((result) => Boolean(result.imageUrl)).map((result) => ({
      result,
    ...
    const qaJobs = visualQaSetting === "1" ? allQaCandidates.map(({ result }) => ({ result })) : selectAdaptiveVisualQaCandidates(allQaCandidates, 2).map(({ result }) => ({ result }));
    ...
    const qaSeen = await mapWithConcurrency(qaJobs, 2, async ({ result: r }) => {
      try {
        ...
        console.warn(`[dev-mode-generation] \xA712H visual-QA call failed`, {
    ...
    for (const entry of qaSeen) {
      ...
      if (entry.regenerate) {
        console.warn(`[dev-mode-generation] \xA712H visual-QA flagged image for regen`, {
...
CORRECTION PASS: ${entry.reasons.join(", ")}. Fix every named issue while preserving the canonical reference identities and exact cast count.`,
        ...
        const duplicateRegeneration = Boolean(regeneratedImageUrl && (regeneratedImageUrl === currentImageUrl || seenGeneratedImageUrls.has(regeneratedImageUrl)));
        ...
        if (regeneratedImageUrl && !duplicateRegeneration) {
          if (currentImageUrl)
            seenGeneratedImageUrls.delete(currentImageUrl);
          ...
          console.log(`[dev-mode-generation] \xA712H visual-QA regen applied`, {
            job: `${r.job.kind}${r.job.order ? `:ch${r.job.order}` : ""}`,
            reasons: entry.reasons
          });
        }
      } catch (err) {
        console.warn(`[dev-mode-generation] \xA712H visual-QA regen failed; keeping original image`, {
          job: `${r.job.kind}${r.job.order ? `:ch${r.job.order}` : ""}`,
          error: err instanceof Error ? err.message : String(err)
        });
      }
...
function hasStrongLocalPotentialMargin(audit) {
  ...
  return !audit.reject && positiveScores.every((score) => Number.isFinite(score) && score >= 8.5) && Number(audit.helperDependencyRisk) <= 5 && Number(audit.similarityToRecentEmotionalMechanics) <= 5;
}
...
function auditIdeaCandidateNovelty(candidate, input) {
  ...
  for (const motif of brief?.hardAvoidMotifs || []) {
    const normalizedMotif = normalizeNoveltyText(motif);
    if (normalizedMotif.length < 6 || isNoveltyStopword(normalizedMotif))
      continue;
...
async function enforceLongTermNovelty(selectedIdea, candidates, input, userId, pool) {
  ...
  try {
    ...
    if (!coreHit) {
      console.log("[dev-mode-generation] \xA73 long-term novelty: clean", {
        candidatesChecked: candidates.length,
        topHitSim: hits[0]?.similarity ?? 0
      });
      return selectedIdea;
    }
    console.warn("[dev-mode-generation] \xA73 long-term novelty: core motif reuse detected, overriding", {
    ...
    if (ranked.length === 0) {
      console.warn("[dev-mode-generation] \xA73 long-term novelty: NO replacement candidate, keeping flagged idea");
      return selectedIdea;
    }
    ...
    console.warn("[dev-mode-generation] \xA73 long-term novelty check failed (non-fatal):", err instanceof Error ? err.message : String(err));
    return selectedIdea;
  }
}
...
function scorePoolCharacterForSelectedIdea(character, selectedIdea, input) {
  ...
  if (setting && canon.some((value) => value === setting || value.includes(setting) || setting.includes(value)))
    score += 16;
  if (genre.includes("fairy") || genre.includes("maerchen") || genre.includes("m\xE4rchen")) {
...
function finalizeSelectedIdeaCast(input, selectedIdea, pool) {
  ...
  const requestedOrder = (selectedIdea.selectedSupportingCast || selectedIdea.recommendedSupportingCast || []).map((name) => String(name || "").trim()).filter(Boolean);
  const requestedSelection = selectLockedSupportingCast(pool, requestedOrder, DEV_MODE_MAX_SUPPORTING_CAST, normalizePoolName);
  const requestedCharacters = requestedSelection.locked;
  const adultExplainerRisks = requestedCharacters.filter((character) => isExplanatoryAdultCastRisk(character, selectedIdea, input)).map((character) => character.name);
  ...
  const scored = pool.map((character) => ({ character, score: scorePoolCharacterForSelectedIdea(character, selectedIdea, input) })).sort((a, b) => b.score - a.score);
  const picked = requestedCharacters.slice(0, targetCount);
  const seenArchetypes = new Set(picked.map((character) => normalizePoolName(character.archetype || "")).filter(Boolean));
  ...
  return {
    selectedIdea: {
      ...
      chosenReason: finalNames.length > 0 ? `${selectedIdea.chosenReason} The locked supporting cast was retained after premise selection so story and image references use the same identities.${adultExplainerRisks.length ? ` Adult helper agency is constrained for: ${adultExplainerRisks.join(", ")}.` : ""}` : `${selectedIdea.chosenReason} No supporting cast was forced after premise selection; main-avatar agency and pacing ranked higher than pool usage.`
    },
...
function getMainAvatarNames(input) {
  const seen = new Set;
  return (input.avatars || []).map((avatar) => String(avatar.name || "").trim()).filter((name) => {
...
function mainAvatarVoiceInstruction(input) {
...
function germanMainAvatarSubject(input) {
  const names = getMainAvatarNames(input);
  const subject = names.length === 0 ? "Die Hauptfigur" : names.length === 1 ? names[0] : names.length === 2 ? `${names[0]} und ${names[1]}` : `${names.slice(0, -1).join(", ")} und ${names[names.length - 1]}`;
  const plural = names.length > 1;
  return {
    subject,
    give: plural ? "geben" : "gibt",
    leave: plural ? "lassen" : "l\xE4sst",
    recognize: plural ? "erkennen" : "erkennt",
    perform: plural ? "f\xFChren" : "f\xFChrt",
...
function voiceForAvatar(avatar) {
  ...
  if (v.logic >= 45 || v.persistence >= 45) {
    fragments.push(`uses short corrections and rules ("X geh\xF6rt zu Y. Immer."), counts on fingers, prefers list-numbers ("Erstens... Zweitens...")`);
  }
  if (v.creativity >= 45) {
  ...
  const tic = fragments.slice(0, 3).join("; ") || `warm observer; short sentences, asks simple "und dann?" questions, reacts with one concrete action per scene (no abstract opinions)`;
  return `${avatar.name}: ${tic}.`;
}
function buildCompactMainAvatarVoiceLock(input) {
  ...
  return [
    ...
    avatars.length >= 2 ? "- Consecutive main-avatar lines must do different jobs. Include one brief, real disagreement about the next action before they decide together; never write agreement echoes." : "- Keep this voice specific in every quoted line; never invent an unplanned co-protagonist."
  ].join(`
`);
}
...
function buildWriterVoiceAnchorBlock(input) {
  ...
  if (code === "de") {
    return [
      ...
      "- PROSE ANTI-TIC: no adjective fragment triplets ('Schwer. Metallisch. Endg\xFCltig.'), no stacked one-word verdicts, no repeated ears-red / fingers-tremble / voice-breaks shorthand. Use one fresh, situation-specific action instead.",
...
function buildSelectedCastIntegrationContract(input, strict = false) {
  ...
  if (castNames.length === 0) {
    ...
    lines.push(`- Selected supporting cast: ${castNames.join(", ")}. Each must change the plot, not merely appear or explain.`, "- For every selected cast figure, include: one visible action only they would do, one line/gesture in their voice, and one causal effect on the problem or solution.", "- Failure condition: if the story still works after deleting that figure, rewrite the beat until the figure is plot-necessary.", "- Cast budget: give each supporting figure a compact scene job. Do not let helpers form a parade, take over the finale, or explain the lesson.", castNames.length > 1 ? "- Split supporting functions clearly. The main avatars must still make the decisive choice." : "- The supporting figure may nudge the problem, but the main avatars must make the decisive choice.");
  }
...
function genreCraftGuidance(genre) {
  ...
  if (normalized.includes("fairy") || normalized.includes("maerchen") || normalized.includes("m\xE4rchen")) {
    return [
...
function languageCodeFromName(languageName) {
  ...
  if (lower.includes("german") || lower.includes("deutsch"))
    return "de";
  if (lower.includes("french") || lower.includes("fran\xE7ais"))
    return "fr";
  if (lower.includes("spanish") || lower.includes("espa\xF1ol"))
    return "es";
...
function targetLanguageStyleAnchor(languageCode) {
  switch (languageCode) {
    ...
    case "fr":
      return "Output-language style contract (French): warm, concrete, sensory; use French guillemets \xAB \u2026 \xBB. This is punctuation/register guidance only, not a story premise.";
    case "es":
      return "Output-language style contract (Spanish): warm, concrete, sensory; use angle quotes \xAB\u2026\xBB. This is punctuation/register guidance only, not a story premise.";
    case "it":
      return "Output-language style contract (Italian): warm, concrete, sensory; use angle quotes \xAB\u2026\xBB. This is punctuation/register guidance only, not a story premise.";
    case "nl":
    ...
    case "ru":
      return "Output-language style contract (Russian): warm, concrete, sensory; use guillemets \xAB\u2026\xBB. This is punctuation/register guidance only, not a story premise.";
    default:
...
function qualitySystemPrompt(languageName, outputSchema) {
  ...
  return [
    ...
    "- 'The greatest gift was friendship.' / 'Das gr\xF6\xDFte Geschenk war Freundschaft.'",
...
function buildLoglineEnginePrompts(input, chapterCount, repairIssues = [], previousEngine) {
  const languageName = localizedLanguageName(input.config.language);
  const systemPrompt = qualitySystemPrompt(languageName, [
    "Schema:",
    "{",
...
function buildSceneCardPrompts(input, beatSheet, repairIssues = []) {
  ...
  const driverOptions = heroNames.length > 0 ? heroNames.map((name) => `"${name}"`).join(", ") : "the named main protagonist";
  const isPremium = (input.qualityMode || "premium") === "premium";
  const systemPrompt = qualitySystemPrompt(languageName, [
    "Schema:",
    "{",
...
`));
  const userPrompt = [
    ...
    "personalCost must name a PHYSICAL object, place, or privilege that is visibly given up, used up, broken, or left behind \u2014 NEVER an abstract feeling. Forbidden: 'verliert Zuversicht', 'muss Scham \xFCberwinden', 'loses hope'. Required shape: 'gibt den letzten X her', 'l\xE4sst Y zur\xFCck', 'Z zerbricht und bleibt zerbrochen'.",
...
function hasClosedSceneEndPull(value) {
  ...
  if (/\b(alles gut|gel\u00F6st)\b/i.test(endPull)) {
    return !["alles gut", "gel\xF6st"].some((word) => isNegated(endPull, word));
  }
  if (/\bruhig\b/i.test(endPull)) {
    return !/\bunruhig\b/i.test(endPull) && !isNegated(endPull, "ruhig");
  }
  if (/\bfertig\b/i.test(endPull)) {
    return !(/\bunfertig\b/i.test(endPull) || /\bfertig\s+(f\u00FCr|zum|mit\s+den\s+nerven)\b/i.test(endPull) || /\bmach(e|t|en|te|ten)?\s+fertig\b/i.test(endPull) || isNegated(endPull, "fertig"));
  }
  return false;
}
function ensureSceneDialogueBeats(card, input) {
  ...
  const templates = [
    { intent: "want", subtext: "will das sichtbare Ziel erreichen", actionCarried: card?.visibleGoal || card?.plant || "zeigt auf den n\xE4chsten Schritt" },
    { intent: "observe", subtext: "bemerkt die Regel oder Gefahr", actionCarried: card?.obstacle || card?.visibleConsequence || "h\xE4lt kurz inne und pr\xFCft die Spur" },
    { intent: "resist", subtext: "will den einfachen Weg nehmen", actionCarried: card?.wrongAction || "greift zu schnell nach der falschen L\xF6sung" },
    { intent: "decide", subtext: "lenkt zur n\xE4chsten Handlung", actionCarried: card?.endPull || card?.childDecision || "setzt eine konkrete Entscheidung in Bewegung" }
  ];
...
function repairSceneCardsDeterministically(sceneCards, beatSheet, input) {
  ...
  const referencesPersonalObject = (value) => {
    const normalized = String(value || "").toLowerCase();
    return personalObjectTokens.length === 0 || personalObjectTokens.some((token) => normalized.includes(token.slice(0, 4)));
  };
  const concretePersonalCost = `${protagonist.subject} ${protagonist.give} ${personalObject} her und ${protagonist.leave} es sichtbar zur\xFCck.`;
  const rawPersonalCost = firstSceneText(beatSheet?.act2?.personalCost, middle.personalCost, input.selectedIdea?.coreConflict);
  const personalCost = rawPersonalCost.length >= 18 && !/(learns|lernt|understands|versteht|erkennt)\b/i.test(rawPersonalCost) && !ABSTRACT_PERSONAL_COST_PATTERN.test(rawPersonalCost) && referencesPersonalObject(rawPersonalCost) ? rawPersonalCost : concretePersonalCost;
  ...
  const visibleDamage = hasConcreteSceneDamageSignal(rawVisibleDamage) ? rawVisibleDamage : `${personalObject} bekommt einen sichtbaren Riss; ein wichtiges St\xFCck verschwindet vor den Augen der Kinder.`;
  ...
  const cannotGoBack = rawCannotGoBack.length >= 18 ? rawCannotGoBack : `Weil ${personalObject} sichtbar besch\xE4digt ist, kann der Anfangszustand nicht einfach durch Warten zur\xFCckkommen.`;
  ...
  const repaired = baseCards.map((card, index) => {
    ...
    if (index < DEV_MODE_SCENE_CARD_COUNT - 1 && hasClosedSceneEndPull(next.endPull)) {
      const openPulls = [
        "Eine neue Spur taucht auf, und die Kinder m\xFCssen ihr sofort folgen.",
        "Bevor sie den n\xE4chsten Schritt pr\xFCfen k\xF6nnen, verschiebt sich das Hindernis erneut.",
        "Die Gefahr w\xE4chst sichtbar; die Kinder m\xFCssen jetzt einen anderen Weg finden.",
        "Die Zeit wird knapp: Nur eine schwierige Entscheidung kann verhindern, dass der Schaden gr\xF6\xDFer wird."
      ];
    ...
    const originalCharacterActions = original.characterActions && typeof original.characterActions === "object" ? original.characterActions : {};
    next.characterActions = Object.fromEntries(heroNames.map((name, avatarIndex) => [
      name,
      firstSceneText(originalCharacterActions[name], original[`${name}Action`], `${name} handelt sichtbar am Objekt.`)
    ]));
    ...
    const safeOriginalCost = ABSTRACT_PERSONAL_COST_PATTERN.test(String(original.personalCost || "")) || !referencesPersonalObject(original.personalCost) ? "" : original.personalCost;
    if (scenePurpose === "irreversible_middle" || index === 2 || index === 3) {
      ...
      next.cannotGoBackReason = firstSceneText(original.cannotGoBackReason, cannotGoBack, "Der alte Zustand ist sichtbar ver\xE4ndert und kann nicht durch Warten zur\xFCckkehren.");
  ...
  const sceneSpecificFields = {
    visibleGoal: [
      ...
      `Im Finale f\xFChrt ${protagonist.subject} die entscheidende Handlung an ${motif} selbst aus.`
    ],
    obstacle: [
      `Die erste sichtbare Regel von ${motif} sperrt den direkten Weg.`,
      `Der bequeme Fehlversuch versch\xE4rft den Widerstand rund um ${personalObject}.`,
      `Die neue Komplikation setzt ${protagonist.subject} am Schauplatz unmittelbar unter Druck.`,
      `Der sichtbare Schaden an ${personalObject} macht eine R\xFCckkehr zum Anfang unm\xF6glich.`,
      `Im Finale verlangt der Konflikt eine konkrete Wahl, die niemand f\xFCr ${protagonist.subject} treffen kann.`
    ],
    wrongAction: [
      `${protagonist.subject} deutet das erste Zeichen zu schnell und greift nach der naheliegenden Abk\xFCrzung.`,
      `${protagonist.subject} wiederholt den bequemen Plan, obwohl ${motif} bereits sichtbar dagegenh\xE4lt.`,
      `${protagonist.subject} versucht die wachsende Komplikation allein mit mehr Kraft zu \xFCberwinden.`,
      `${protagonist.subject} will den Preis umgehen und ${personalObject} zugleich festhalten.`,
      `Kurz vor der Entscheidung erw\xE4gt ${protagonist.subject} noch einmal den alten, folgenlosen Ausweg.`
    ],
    visibleConsequence: [
      `Das erste Zeichen an ${motif} ver\xE4ndert sich sichtbar und setzt die Handlung in Gang.`,
      `Der Fehlversuch verschiebt ${personalObject} und macht das n\xE4chste Hindernis gr\xF6\xDFer.`,
      `Die Komplikation hinterl\xE4sst am Schauplatz eine neue sichtbare Spur.`,
      `${visibleDamage}`,
    ...
    endPull: [
      ...
      `Der Fehlversuch l\xF6st eine neue Bewegung aus, bevor ${protagonist.subject} eingreifen kann.`,
      `Am Rand des Schauplatzes beginnt ${personalObject} sich sichtbar zu ver\xE4ndern.`,
      `Jetzt bleibt nur die konkrete Wahl aus dem Finale, und die Zeit daf\xFCr wird knapp.`,
...
function repairLoglineEngineDeterministically(engine, languageCode) {
  ...
  if (!refrain || refrainLooksExpository(refrain)) {
    const motifWords = motif.split(/[^\p{L}]+/u).filter((w) => w.length >= 2 && !refrainLooksExpository(w)).slice(0, 3);
    ...
    repaired.refrainLine = candidate.length > 0 ? `${candidate}!` : fallbackByLang[lang] || fallbackByLang.en;
  }
  return repaired;
}
...
function repairBeatSheetDeterministically(beatSheet, input, issues) {
  ...
  const personalContractIssue = issues.some((issue) => /personalObject|personalCost|finalPayoff\.plantedDetail|irreversibleMiddle\.personalCost/i.test(issue));
  const midpointIrreversibilityIssue = issues.some((issue) => /midpointIrreversibleTurn is not visibly irreversible/i.test(issue));
  if (personalContractIssue || midpointIrreversibilityIssue) {
    const objectTokens = normalizeNoveltyText(objectName).split(/[^a-z0-9]+/).filter((token) => token.length >= 4).map((token) => token.slice(0, 4));
    const referencesObject = (value) => {
    ...
    const keepAnchored = (value, fallback) => {
      const text = String(value || "").trim();
    ...
    const shouldReplaceAct2Cost = issues.some((issue) => /act2\.personalCost|personalCost is not concrete|mood\/insight/i.test(issue));
    const currentPersonalObject = beatSheet.personalObject && typeof beatSheet.personalObject === "object" ? beatSheet.personalObject : {};
    ...
    const safeAct2Cost = shouldReplaceAct2Cost || ABSTRACT_PERSONAL_COST_PATTERN.test(currentAct2Cost) ? concreteCost : keepAnchored(currentAct2Cost, concreteCost);
    const safeMiddleCost = ABSTRACT_PERSONAL_COST_PATTERN.test(currentMiddleCost) ? concreteCost : keepAnchored(currentMiddleCost, safeAct2Cost);
    repaired = {
      ...repaired,
      personalObject: {
      ...
      act2: {
        ...beatSheet.act2 || {},
        midpointIrreversibleTurn: midpointIrreversibilityIssue ? visibleDamage : keepAnchored(beatSheet?.act2?.midpointIrreversibleTurn, visibleDamage),
  ...
  sceneCards.forEach((card, index) => {
    ...
    if (index < sceneCards.length - 1) {
      ...
      if (!isTooClosed && /\b(alles gut|gel\u00F6st)\b/i.test(endPullVal)) {
        if (!["alles gut", "gel\xF6st"].some((w) => isNegated(endPullVal, w))) {
          isTooClosed = true;
        }
      }
...
function compactScreenplayPlanForDraft(plan) {
  ...
  return {
    ...
    sceneCards: sceneCards.map((card) => ({
      ...
      castMoves: {
        ...Object.fromEntries(Object.entries(card.characterActions || {}).map(([name, action]) => [name, compactExcerpt(String(action || ""), 90)]).filter(([, action]) => Boolean(action))),
        ...card.helperAction ? { supportingCharacter: compactExcerpt(card.helperAction, 90) } : {}
      },
...
function buildCompactValidationNoveltyBlock(input) {
  ...
  const trueHardAvoid = brief.hardAvoidMotifs.filter((motif) => !isNoveltyStopword(normalizeNoveltyText(motif))).filter(isHardBanMotif).slice(0, 14);
  const softMotifHints = brief.hardAvoidMotifs.filter((motif) => !isNoveltyStopword(normalizeNoveltyText(motif))).filter((motif) => !isHardBanMotif(motif)).slice(0, 14);
...
`);
  ...
  const userPrompt = [
    ...
    '- ending is an IMAGE, not a moral. No "sie lernten..." / "they learned...". Also no as-if moral similes ("als w\xE4re er kl\xFCger geworden", "as if it had grown wiser") \u2014 the last sentence shows one concrete object, sound, or motion in the scene.',
    "",
    ...
    mainAvatarVoiceInstruction(input),
    "",
    languageCodeFromName(languageName) === "de" ? 'SPRACH-ECHTHEIT (Pflicht): Nur echte, gebr\xE4uchliche deutsche W\xF6rter. KEINE erfundenen Komposita oder Kunstw\xF6rter (kein "apfelscharf" o.\xE4.). Wenn unsicher, ob ein 7-J\xE4hriger das Wort kennt: einfacheres Wort w\xE4hlen. Vergleiche aus der Kinderwelt (Spielzeug, Tiere, Essen, Wetter).' : "LANGUAGE AUTHENTICITY (mandatory): only real, common words a 7-year-old knows. Never invent compound words. If unsure, choose the simpler word.",
    "",
...
`),
    ...
    '- The child makes a clearly conscious choice ("Ich gebe ... her", "Ich lasse ... zur\xFCck", "Wir lassen ... so wie es ist") \u2014 not an accidental success, not a discovery by the helper.',
...
`) : "";
  ...
  const userPrompt = [
    ...
    hasNoveltyIssue ? "- NOVELTY WORDING PRIORITY: remove or replace validator-listed repeated/forbidden wording if it is not load-bearing. For generic German words like 'zur\xFCck', use 'wieder', 'nach Hause', 'vorbei', or a concrete movement instead." : null,
    "",
    ...
    languageCodeFromName(localizedLanguageName(input.config.language)) === "de" ? '- SPRACH-ECHTHEIT: nur echte, gebr\xE4uchliche deutsche W\xF6rter. Ersetze erfundene Komposita/Kunstw\xF6rter (z.B. "apfelscharf") durch echte kindgerechte W\xF6rter ("klein wie eine Erbse").' : null,
    "",
    "PAYOFF CONTRACT:",
...
`));
  ...
  const userPrompt = [
    ...
    "- generic narrator description that could be in any children's book ('Es war sch\xF6n.', 'Sie waren gl\xFCcklich.')",
    ...
    "- Replacement should be roughly the SAME LENGTH as the original (\xB140 characters). Do not grow chapter length.",
...
function selectValidatorQualityRepairChapters(diagnostics, validatorFindings, chapterCount) {
  ...
  const add = (order) => {
    if (!order || selected.size >= selectionLimit)
      return;
  ...
  for (const finding of continuityFindings) {
    const pageNumbers = [...finding.matchAll(/(?:page|reading page|leseseite|chapter|kapitel)\s*(\d+)/gi)].map((match) => Number(match[1])).filter((order) => order >= 1 && order <= chapterCount);
    if (pageNumbers.length > 0)
      add(Math.max(...pageNumbers));
  }
...
function validatorAnchorBlock(languageCode) {
  if (languageCode === "de") {
    return [
      ...
      `  \u201EAnna nahm den goldenen Schl\xFCssel und sagte: 'Damit \xF6ffnen wir das geheimnisvolle Tor!' Ben nickte tapfer. Gemeinsam st\xFCrmten sie los, denn sie wussten: Freundschaft ist st\xE4rker als jede Angst."`,
      ...
      `  \u201ELena und Tom betraten den verzauberten Wald. Die B\xE4ume schimmerten in allen Farben. 'Wir m\xFCssen mutig sein!', rief Lena. Sie hatten gelernt, dass wahre Magie im Herzen liegt."`,
      ...
      '  \u201ESie gingen weiter und weiter. Pl\xF6tzlich sahen sie einen Drachen. Sie hatten Angst. Aber dann waren sie mutig und freundeten sich mit dem Drachen an. Alle waren gl\xFCcklich."',
...
`);
  ...
  for (const motif of brief.hardAvoidMotifs) {
    ...
    if (normalizedMotif.length < 6)
      continue;
    if (isNoveltyStopword(normalizedMotif))
      continue;
    if (isCurrentCharacterNameMotif(normalizedMotif, input))
      continue;
    if (explicitSoundRequest && /gloeckchen|glocke|bell|sound|klang|geraeusch|stille|lautlos/.test(normalizedMotif)) {
...
var TITLE_PROMISE_SYNONYMS_DE = {
  ...
  jahres: ["jahres", "frueh", "fr\xFCh", "sommer", "herbst", "winter"]
};
...
`);
  ...
  if (serializationArtifacts.detected) {
    ...
    polishInstructions.push("Nicht lokal bereinigen: verwirf den strukturell besch\xE4digten Entwurf vollst\xE4ndig und generiere ihn erneut als valides Story-JSON.");
  }
  ...
  if (helperNames.length > 0 && languageCode === "de") {
    ...
    if (triggeredChapters.length > 0) {
      ...
      const isPremium = (input.qualityMode || "premium") === "premium";
      const message = `Helper-Explains-Gate: ${firstHelper} erklaert die Magieregel/Loesung direkt im Dialog (${firstEvidence}) in Kapitel ${triggeredChapters.join(",")}. Helfer d\xFCrfen scheitern, st\xF6ren oder ein Werkzeug geben \u2014 nicht die Magieregel oder L\xF6sung aussprechen.`;
      if (isPremium && (triggeredChapters.length >= 2 || finaleAdjacent)) {
        hardIssues.push(message);
      } else {
        softIssues.push(message);
      }
      polishInstructions.push("Lass Helfer NICHT die Magieregel oder L\xF6sung erkl\xE4ren. Stattdessen: Helfer gibt nur ein Werkzeug, Druck oder eine missverst\xE4ndliche Geste; die Kinder m\xFCssen die Regel selbst aus wiederholten Folgen herausfinden.");
    }
  }
  if (languageCode === "de") {
    ...
    if (grammar.hardIssues.length > 0) {
      polishInstructions.push("Behebe Grammatik-Artefakte vollst\xE4ndig (z.B. 'Ich Idee' \u2192 'Ich habe eine Idee'; 'Der ist silberne' \u2192 'Der ist silbern').");
    }
  }
  if (story.chapters.length >= 3 && languageCode === "de") {
    ...
    if (!structure.hasIrreversibleMiddle) {
      softIssues.push("Strukturelle Schw\xE4che: keine sichtbare irreversible Mitte (Verlust, sichtbare Ver\xE4nderung, Schrumpfen) erkannt.");
    }
    if (!structure.hasPersonalSacrifice) {
      softIssues.push("Strukturelle Schw\xE4che: kein pers\xF6nlicher Einsatz/Opfer erkannt (Figur gibt etwas Geliebtes her).");
    }
    if (!structure.finaleEndsInImage) {
      softIssues.push("Finale endet eher mit Erkl\xE4rung als mit Bild/Handlung; baue konkrete Schlussbeobachtung ein.");
    }
  }
  for (const seam of detectChapterSeamDuplicates(story.chapters.map((c) => ({ order: c.order, content: c.content })))) {
    softIssues.push(`Naht-Duplikat: Leseseite ${seam.order} beginnt mit denselben S\xE4tzen, mit denen Leseseite ${seam.previousOrder} endet ("${seam.sentences[0]}"). Entferne die Wiederholung am Seitenanfang.`);
    polishInstructions.push("Entferne wortgleiche Satzwiederholungen an Seitengrenzen: Ein Seitenanfang darf die Schlusss\xE4tze der vorherigen Seite nicht erneut erz\xE4hlen; formuliere den Anschluss als neue Bewegung.");
  }
  ...
  if (dialogPct < minDialogPct) {
    ...
    softIssues.push(`Dialoganteil ist mit ${dialogPct}% zu hoch (Obergrenze ${DEV_MODE_MAX_DIALOG_PCT}%); ersetze einen Teil der w\xF6rtlichen Rede durch sinnliche Erz\xE4hlung, Handlung und innere Wahrnehmung, damit die Vorlese-Melodie nicht in reinem Geplauder untergeht.`);
  }
...
function applyDeterministicStoryTextAutofixes(story, input) {
  ...
  const fixedStory = story.displayMode === "reading_pages" ? markStoryAsReadingPages({ ...story, title: fixedTitle, description: fixedDescription, chapters: fixedChapters }, story) : { ...story, title: fixedTitle, description: fixedDescription, chapters: fixedChapters };
...
function titleEditDistance(left, right) {
...
function applySafeValidatorCorrectedTitle(story, validatorFindings) {
  ...
  if (!candidate || candidate === current)
    return current !== story.title ? { ...story, title: current } : story;
  ...
  if (Math.abs(currentWords.length - candidateWords.length) > 1)
    return story;
  ...
  if (titleEditDistance(currentCompact, candidateCompact) > allowedDistance)
    return story;
...
function releaseBlockingValidatorMustFixes(validatorFindings, mode, diagnostics) {
  if ((mode || "premium") !== "premium")
    return [];
  ...
  return mustFixes.filter((fix) => materialIssuePattern.test(fix)).filter((fix) => !(/dialog|direct speech|direkte rede/i.test(fix) && !structuralIssuePattern.test(fix) && (diagnostics?.dialogPct ?? 0) >= DEV_MODE_MIN_DIALOG_PCT)).filter((fix) => {
...
function isCompletionLengthFailure(error) {
  const message = (error instanceof Error ? error.message : String(error ?? "")).toLowerCase();
  return message.includes("finish_reason=length") || message.includes("finish_reason=max_tokens") || message.includes("empty response from openrouter");
}
...
async function generateStoryDevMode(input) {
  ...
  if (writerModelFloor) {
    console.log("[dev-mode-generation] \xA75 writer-model floor active for all writer stages", {
  ...
  let selectedIdea;
  let screenplayPlan;
  let bestValidatedSnapshot;
  const considerValidatedSnapshot = () => {
    if (!finalParsed || !finalDiagnostics || !finalValidatorFindings)
      return;
    const signals = {
      hardIssueCount: finalDiagnostics.hardIssueCount ?? 0,
      errorCount: asStringArray(finalValidatorFindings.errors, 20).length,
      blockerCount: asStringArray(finalValidatorFindings.publishabilityBlockers, 20).length + asStringArray(finalValidatorFindings.continuityBreaks, 20).length,
      releaseFailureCount: releaseDimensionFailures(finalValidatorFindings, { mode: input.qualityMode }).length + releaseBlockingValidatorMustFixes(finalValidatorFindings, input.qualityMode, finalDiagnostics).length,
  ...
  try {
    ...
    let lastUsableIdeaCandidates = ideaCandidates;
    {
      ...
      for (let ideaRound = 1;ideaRound <= DEV_MODE_MAX_IDEA_ROUNDS && !selectedIdea; ideaRound += 1) {
        ...
        const localPotentialFilter = normalizePotentialFilterResult({}, potentialFilterCandidates, input, input.poolCharacters);
        const localChosenAudit = localPotentialFilter.candidateAudits.find((audit) => audit.id === localPotentialFilter.chosenIdeaId);
      ...
      if (!selectedIdea && ideaCandidates.length > 0) {
        ...
        if (bestCandidate && !shouldBlockPotentialGate) {
          ...
          console.warn(`[dev-mode-generation] \xA74 ${lane} soft-fail: no candidate passed gate after ${DEV_MODE_MAX_IDEA_ROUNDS} rounds; using best-audit fallback`, {
          ...
          selectedIdea = {
            ...bestCandidate,
            chosenReason: `\xA74 ${lane} soft-fail: best of ${ideaCandidates.length} candidates after ${DEV_MODE_MAX_IDEA_ROUNDS} strict rounds. ${bestAuditLine}`,
          ...
          console.warn("[dev-mode-generation] \xA74 PREMIUM strict-fail: no candidate passed thresholds after 2 rounds \u2014 refusing soft-fail", {
      ...
      if (!selectedIdea) {
        ...
        if (emergencyFallback) {
          console.warn("[dev-mode-generation] \xA74 emergency fallback: no passing idea candidate survived, but strict blocking is disabled", {
      ...
      if (!selectedIdea) {
        ...
        if (mode === "premium" && shouldBlockDevModePotentialGateFailure(input)) {
          throw new Error(`\xA72 premium quality_gate_failed: no candidate met the strict thresholds after ${DEV_MODE_MAX_IDEA_ROUNDS} round(s). Caller must regenerate with a different creative lane. Last failures: ${potentialFailureSummaries.slice(0, 6).join(" | ")}`);
        }
        ...
        if (deterministicFallback) {
          console.warn("[dev-mode-generation] \xA74 last-resort deterministic idea fallback after empty/invalid candidate pool", {
      ...
      if (!selectedIdea) {
        ...
        try {
          ...
          console.warn("[dev-mode-generation] \xA73 long-term novelty enforcement failed (non-fatal):", longTermErr instanceof Error ? longTermErr.message : String(longTermErr));
        }
        ...
        if (selectedIdea && ideaCandidates.length > 0) {
          ...
          console.log("[dev-mode-generation] \xA74 candidate-9.0 audit", {
          ...
          if (selectedAudit?.audit.reject) {
            ...
            if (replacement) {
              ...
              if (replacementCandidate) {
                console.warn("[dev-mode-generation] \xA74 candidate-9.0 swap", {
                ...
                selectedIdea = {
                  ...
                  chosenReason: `\xA74 candidate-9.0 swap: original "${selectedIdea.title}" failed structural gate (${selectedAudit.audit.rejectReason}). Switched to "${replacement.title}".`,
    ...
    if (selectedIdea) {
      ...
      if (hardAvoidForPreflight.length > 0 && selectedIdea) {
        ...
        if (preflight.softWarnings.length > 0) {
          console.log("[dev-mode-generation] \xA73 idea novelty drift (soft)", {
            warnings: preflight.softWarnings.slice(0, 6)
          });
        }
        if (preflight.violations.length > 0) {
          console.warn("[dev-mode-generation] \xA73 forbidden-motif preflight on selected idea (HARD)", {
          ...
          if (preflight.canRepair && preflight.repairedPlan) {
            ...
            selectedIdea = {
              ...
              chosenReason: `${selectedIdea.chosenReason || ""} | \xA73 motif-preflight rename: ${preflight.repairLog.join("; ")}`.trim()
            };
    ...
    if (loglineIssues.length > 0) {
      ...
      const repairedEngine = applySuppressedArtifactSanitizer("logline-emotional-engine-repair", repairedLoglineStage.parsed || loglineEngine);
    ...
    if (beatSheetIssues.length > 0) {
      ...
      if (advisory.length > 0) {
        console.warn("[dev-mode-generation] beat-sheet \xA7H additive checks downgraded to advisory", {
          advisory
        });
      }
    ...
    if (sceneCardIssues.length > 0) {
      ...
      if (blockingIssues.length > 0) {
        ...
        const safeFallbackBlocking = hardPremium ? safeFallbackIssues : safeFallbackIssues.filter((issue) => !isAdvisoryUnderEfficient(issue));
        if (safeFallbackBlocking.length > 0) {
          throw new Error(`Scene-card internal fallback invariant failed: ${safeFallbackBlocking.join(" | ")}; original model issues: ${blockingIssues.join(" | ")}`);
        }
      ...
      if (sceneCardIssues.length > 0) {
        console.warn("[dev-mode-generation] \xA7I efficient mode soft-fail on additive scene-card gates", {
    ...
    if (hardAvoidForSceneScan.length > 0 && Array.isArray(sceneCards) && sceneCards.length > 0) {
      ...
      if (sceneFlight.softWarnings.length > 0) {
        console.log("[dev-mode-generation] \xA73 scene-cards novelty drift (soft)", {
          warnings: sceneFlight.softWarnings.slice(0, 6)
        });
      }
      if (sceneFlight.violations.length > 0) {
        console.warn("[dev-mode-generation] \xA73 forbidden-motif preflight on scene cards (HARD)", {
    ...
    }
    {
      ...
      if (!hasVisibleDamage || !hasPersonalCost) {
        console.warn("[dev-mode-generation] \xA76 irreversible-middle gate flagged", {
          turnIndex,
          hasVisibleDamage,
          hasPersonalCost,
        ...
        const repairIssues = [
          !hasVisibleDamage ? `Scene ${turnIndex + 1} (Wendepunkt) hat keinen sichtbaren Schaden \u2014 f\xFCge visibleDamage konkret (Objekt, Kratzer, Riss, Sto\xDF) hinzu.` : null,
          !hasPersonalCost ? `Scene ${turnIndex + 1} (Wendepunkt) hat keinen pers\xF6nlichen Einsatz \u2014 f\xFCge personalCost konkret (Lieblingsplatz, Murmel, Tuch, Verzicht) hinzu.` : null
        ...
        if (stillNoDamage || stillNoCost) {
          throw new Error(`Scene-card \xA76 gate still failed after repair: visibleDamage=${!stillNoDamage}, personalCost=${!stillNoCost} on scene ${retryTurnIndex + 1}.`);
        }
      }
    }
    ...
    if (useCompactWholeStoryDraft) {
      console.log("[dev-mode-generation] \xA710 compact whole-story-draft prompt enabled", {
    ...
    let selectedStoryChapterRepairLengthFailure;
    const runChapterRepairStage = async (prompts, options) => {
      ...
      try {
        ...
        if (!isCompletionLengthFailure(error))
          throw error;
    ...
    while (finalDiagnostics?.needsPolish && repairAttempt < maxRepairAttempts) {
      ...
      console.log("[dev-mode-generation] \xA79/\xA7O RepairRouter decision", {
        attempt: repairAttempt,
        attemptOf: maxRepairAttempts,
        strategy: routerDecision.strategy,
        reason: routerDecision.reason,
      ...
      if (finalParsed?.displayMode === "reading_pages") {
        ...
        if (shouldRunReadingPageRebalance) {
          const targetLimit = finalDiagnostics.dialogPct < DEV_MODE_BROAD_DIALOG_REPAIR_PCT ? finalParsed.chapters.length : 2;
          ...
          try {
            ...
            const rebalanceStage = await runStage("dialogue-rebalance", rebalancePrompts, {
              ...
              temperature: 0.22,
              timeoutMs: targetLimit > 2 ? input.config.length === "long" ? 220000 : 180000 : 120000,
              modelRole: "selected-story"
            });
      ...
      if (isDeterministicRepairStrategy(routerDecision.strategy)) {
        console.log("[dev-mode-generation] \xA7O skipping chapter-repair LLM call \u2014 deterministic remediation handles this", {
          strategy: routerDecision.strategy
        });
        break;
      }
      ...
      for (const chapterDiagnostic of chaptersToRepair) {
        ...
        if (repairSelfReflectionUnreliable) {
          console.warn("[dev-mode-generation] \xA710 model self-reflection unreliable; deterministic verdict wins", {
            attempt: repairAttempt,
            order: repairResult.chapter.order,
            title: repairResult.chapter.title,
    ...
    for (let validationAttempt = 0;validationAttempt <= DEV_MODE_MAX_VALIDATION_POLISH_ATTEMPTS; validationAttempt += 1) {
      ...
      const goodEnoughToStop = !hasLocalHardIssues && validatorMaterialFailures.length === 0 && currentScore >= DEV_MODE_DIMINISHING_RETURNS_SCORE;
      const shouldAttemptStoryPolish = (validationAttempt < DEV_MODE_MAX_VALIDATION_POLISH_ATTEMPTS || lastRejectedPolishFeedback !== null) && Boolean(finalParsed) && Boolean(finalDiagnostics) && (hasLocalHardIssues || currentScore < targetReleaseScore) && !goodEnoughToStop;
      ...
      try {
        ...
        if (finalParsed?.displayMode === "reading_pages" && Array.isArray(finalParsed.chapters) && finalParsed.chapters.length !== chapterCount) {
          console.warn("[dev-mode-generation] \xA74 page-count repair: polish drifted from required count", {
            requiredPageCount: chapterCount,
          ...
          if (rebuiltParagraphs.length >= chapterCount) {
            ...
            console.log("[dev-mode-generation] \xA74 page-count repair applied", {
              afterPageCount: finalParsed.chapters.length,
              afterTotalWords: finalDiagnostics.totalWords
            });
          } else {
            console.warn("[dev-mode-generation] \xA74 page-count repair skipped: too few paragraphs to split", {
    ...
    if (finalParsed) {
      ...
      if (sanitized.changed && sanitized.description.length >= 10) {
        console.log("[dev-mode-generation] \xA72 metadata sanitizer applied", {
      ...
      if (orthoFixes.length > 0) {
        console.log("[dev-mode-generation] \xA78 orthography autofix applied", {
      ...
      if (finalParsed.displayMode === "reading_pages") {
        ...
        if (rebalance.changed) {
          ...
          console.log("[dev-mode-generation] \xA713 reading-page layout rebalanced", {
...
`);
        ...
        for (const motif of brief.hardAvoidMotifs) {
          const normalizedMotif = normalizeNoveltyText(motif);
          if (normalizedMotif.length < 4 || isNoveltyStopword(normalizedMotif))
            continue;
          if (isCurrentCharacterNameMotif(normalizedMotif, input))
            continue;
  ...
  if (isDebugMode(input)) {
    console.warn("[dev-mode-generation] \xA7B debug mode: skipping image generation (text-only diagnostics run)");
  ...
  if (!input.disablePersistenceSideEffects && input.storyId && input.userId) {
    try {
      ...
      console.warn("[dev-mode-generation] \xA73 recordStoryMotif failed (non-fatal):", err instanceof Error ? err.message : String(err));
    }
  }
  ...
  return {
    ...
    metadata: {
      ...
      releaseReady: (finalDiagnostics?.hardIssueCount ?? 0) === 0 && releaseDimensionFailures(finalValidatorFindings, { mode: input.qualityMode }).length === 0 && releaseBlockingValidatorMustFixes(finalValidatorFindings, input.qualityMode, finalDiagnostics).length === 0 && releasePremiumStrictGate(finalDiagnostics, input.qualityMode).length === 0 && (finalQualityScore ?? rawQualityScore ?? localGateScore ?? 0) >= minReleaseScoreForMode(input.qualityMode),
      status: (() => {
        const mode = input.qualityMode || "premium";
        const releaseScore = finalQualityScore ?? rawQualityScore ?? localGateScore ?? 0;
        const minScore = minReleaseScoreForMode(mode);
        const releaseReadyComputed = (finalDiagnostics?.hardIssueCount ?? 0) === 0 && releaseDimensionFailures(finalValidatorFindings, { mode }).length === 0 && releaseBlockingValidatorMustFixes(finalValidatorFindings, mode, finalDiagnostics).length === 0 && releasePremiumStrictGate(finalDiagnostics, mode).length === 0 && releaseScore >= minScore;
