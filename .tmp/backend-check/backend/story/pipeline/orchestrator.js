...
import { runSemanticCritic } from "./semantic-critic";
...
import { runSoulGate } from "./story-soul-validator";
...
var BLOCKING_IMAGE_ISSUE_CODES = new Set(["SCHEMA", "REF_COUNT", "REF_EXTRA", "TOO_MANY_PROPS"]);
function buildSupplementalScenicImageSpecs(input) {
  ...
  return input.imageSpecs.map((spec) => {
    ...
    return {
      ...
      setting: directive?.setting || spec.setting,
      sceneDescription: directive ? `${directive.goal || ""} ${directive.conflict || ""} ${directive.outcome || ""}`.trim() : "",
      refs: {},
...
class StoryPipelineOrchestrator {
  deps;
  storyWriter;
  imageDirector;
  imageGenerator;
  ...
  constructor(deps = {}) {
    this.deps = deps;
    this.storyWriter = deps.storyWriter || new LlmStoryWriter;
    this.imageDirector = deps.imageDirector || new TemplateImageDirector;
    this.imageGenerator = deps.imageGenerator || new RunwareImageGenerator;
    this.visionValidator = deps.visionValidator || new SimpleVisionValidator;
  }
  async run(input) {
    ...
    const selectedStoryModel = resolveConfiguredStoryModel(normalized.rawConfig);
    ...
    normalized.wordBudget = computeWordBudget({
      ...
      releaseMode: normalized.rawConfig?.releaseMode
    });
    const stylePack = await loadStylePack({ language: normalized.language, category: normalized.category });
    const parentalGuidance = typeof normalized.rawConfig?.parentalGuidance === "string" ? normalized.rawConfig.parentalGuidance.trim() : "";
    const stylePackText = [
      formatStylePackPrompt(stylePack),
      parentalGuidance ? `PARENTAL SAFETY RULES:
${parentalGuidance}` : ""
    ].filter(Boolean).join(`
...
`);
    ...
    let qualityReport;
    let criticReport;
    let blueprintResultV8;
    let storySoul;
    let soulGateResult;
    try {
      if (normalized.category === "Klassische M\xE4rchen" && !normalized.taleId) {
        try {
          const rawConfig = normalized.rawConfig;
      ...
      const variantSeed = normalized.variantSeed ?? crypto.randomInt(0, 2147483647);
      const phase1Start = Date.now();
      const blueprint = await loadStoryBlueprintBase({ normalized, variantSeed });
      await logPhase("phase1-dna", { storyId: normalized.storyId, taleId: "taleId" in blueprint.dna ? blueprint.dna.taleId : blueprint.dna.templateId }, { durationMs: Date.now() - phase1Start, roleCount: blueprint.roles.length, sceneCount: blueprint.scenes.length, title: "title" in blueprint.dna ? blueprint.dna.title : "Template-based" });
      ...
      await upsertStoryInstance({
        ...
        taleId: "taleId" in blueprint.dna ? blueprint.dna.taleId : blueprint.dna.templateId,
      ...
      await logPhase("phase0-normalization", { storyId: normalized.storyId, category: normalized.category, language: normalized.language, chapterCount: normalized.chapterCount }, { ok: true, durationMs: Date.now() - phase0Start, avatarCount: normalized.avatarCount, ageMin: normalized.ageMin, ageMax: normalized.ageMax, aiProvider: normalized.rawConfig?.aiProvider, aiModel: normalized.rawConfig?.aiModel, openRouterModel: normalized.rawConfig?.openRouterModel, selectedStoryModel });
      ...
      if (pipelineConfig.soulStageEnabled && !pipelineConfig.soulStageDisabled) {
        ...
        try {
          ...
          const gateSuccess = !pipelineConfig.soulGateEnabled || soulGateResult?.verdict === "approved" || soulGateResult?.verdict === "acceptable_with_warnings";
          const combinedIssues = [
            ...(soulGateResult?.schemaIssues ?? soulGeneration.issues).map((issue) => ({ ...issue })),
            ...(soulGateResult?.rubricScores ?? []).filter((s) => s.score < 7).map((s) => ({
          ...
          if (!gateSuccess && !pipelineConfig.soulAllowOnReject) {
            throw new Error(`Story Soul gate rejected (${soulGateResult?.verdict ?? "schema_only"}): ${(soulGateResult?.repairInstruction ?? "Soul schema/rubric failed").slice(0, 240)}`);
          }
        } catch (soulError) {
          const soulSoftFail = pipelineConfig.soulAllowOnReject;
          console.warn(soulSoftFail ? "[pipeline] \u26A0\uFE0F Soul stage failed, falling back to pre-soul pipeline:" : "[pipeline] \u26A0\uFE0F Soul stage failed, aborting in quality-first mode:", soulError);
          storySoul = undefined;
      ...
      let storyDraft = { title: "", description: "", chapters: [] };
      let blueprintV8;
      let releaseReport;
      const releaseEnabled = normalized.rawConfig?.releaseMode !== false;
      const configuredCandidateCount = Number(pipelineConfig.releaseCandidateCount ?? 1);
      const explicitCandidateCount = Number(normalized.rawConfig?.releaseCandidateCount);
      ...
      const configuredDefaultSingleCandidate = !hasExplicitCandidateCount && Number.isFinite(configuredCandidateCount) && Math.round(configuredCandidateCount) <= 1;
      const defaultCandidateCount = Number.isFinite(configuredCandidateCount) ? Math.max(1, Math.min(3, Math.round(configuredCandidateCount))) : 2;
      const qualityFirstV8Lane = activePromptVersion === "v8" && normalized.language === "de" && normalized.ageMax <= 8 && directives.length === 5;
      const implicitCandidateFloor = hasExplicitCandidateCount ? 1 : configuredDefaultSingleCandidate ? 1 : qualityFirstV8Lane ? 2 : 1;
      const preliminaryCandidateCount = releaseEnabled ? Math.max(implicitCandidateFloor, Math.min(3, hasExplicitCandidateCount ? Math.round(explicitCandidateCount) : defaultCandidateCount)) : 1;
      const soulApprovedForSingleCandidate = pipelineConfig.soulApprovedSingleCandidate && (storySoul && !pipelineConfig.soulGateEnabled || soulGateResult?.verdict === "approved" || soulGateResult?.verdict === "acceptable_with_warnings") && !Number.isFinite(explicitCandidateCount);
      const releaseCandidateCount = soulApprovedForSingleCandidate ? 1 : preliminaryCandidateCount;
      const adaptiveSecondCandidateRaw = normalized.rawConfig?.enableAdaptiveSecondCandidate;
      const enableAdaptiveSecondCandidate = typeof adaptiveSecondCandidateRaw === "boolean" ? adaptiveSecondCandidateRaw : Boolean(pipelineConfig.enableAdaptiveSecondCandidate) && qualityFirstV8Lane && releaseCandidateCount === 1 && !userRequestsSingleCandidate;
      const adaptiveSecondCandidate = releaseEnabled && !hasExplicitCandidateCount && releaseCandidateCount === 1 && enableAdaptiveSecondCandidate;
      const criticModel = resolveCriticModelForPipeline({
        selectedStoryModel,
        explicitCriticModel: String(normalized.rawConfig?.criticModel || ""),
        defaultModel: String(pipelineConfig.criticModel || "gemini-3.1-flash-lite-preview")
      });
      const baseMinScore = clampNumber(Number(pipelineConfig.pass3TargetScore ?? 8), 5.5, 10);
      const soulAwareMinScore = clampNumber(Number(pipelineConfig.soulAwareCriticMinScore ?? baseMinScore), 5.5, 10);
      const criticMinScore = soulGateResult?.verdict === "approved" || soulGateResult?.verdict === "acceptable_with_warnings" ? Math.max(baseMinScore, soulAwareMinScore) : baseMinScore;
      const criticWarnFloor = clampNumber(Number(pipelineConfig.pass3WarnFloor ?? 6.5), 5, criticMinScore);
      const rawSurgeryEdits = normalized.rawConfig?.maxSelectiveSurgeryEdits;
      ...
      const implicitSurgeryEdits = Number.isFinite(configuredSurgeryEdits) ? Math.max(0, Math.min(5, Math.round(configuredSurgeryEdits))) : 2;
      const implicitSurgeryFloor = 1;
      const maxSelectiveSurgeryEdits = hasExplicitSurgeryOverride && Number.isFinite(explicitSurgeryEdits) ? Math.max(0, Math.min(5, explicitSurgeryEdits)) : Math.max(implicitSurgeryFloor, implicitSurgeryEdits);
      const surgeryEnabled = releaseEnabled && maxSelectiveSurgeryEdits > 0;
      const humorLevel = typeof normalized.rawConfig?.humorLevel === "number" ? normalized.rawConfig.humorLevel : 2;
      const avatarMemories = new Map;
      try {
        for (const avatar of input.avatars) {
          const rows = [];
          const gen = await avatarDB.query`
            WITH ranked AS (
              SELECT
                id,
                story_title,
            ...
            SELECT id, story_title, emotional_impact, summary
            FROM ranked
              ...
              await avatarDB.exec`
                ...
                SET recall_count = COALESCE(recall_count, 0) + 1,
                    ...
                    memory_tier = CASE
                      ...
                      ELSE memory_tier
                    END
                WHERE id = ${memory.id} AND avatar_id = ${avatar.id}
              `;
            } catch (recallError) {
              console.warn("[pipeline] Could not update memory recall metadata", recallError);
            }
          }
          if (rows.length > 0) {
            const compactMemories = rows.map((r) => ({
              storyTitle: sanitizeMemoryTitleForPrompt(r.story_title || ""),
              experience: sanitizeMemoryExperienceForPrompt(r.summary || ""),
              emotionalImpact: r.emotional_impact || "neutral"
      ...
      if (activePromptVersion === "v8" && storedText.length !== directives.length) {
        ...
        const iconicMotif = blueprintV8?.iconic_motif;
        if (iconicMotif?.object) {
          ...
          directives = directives.map((directive, idx) => {
            ...
            const motifWithContext = positionHint ? `${iconicMotif.object} (${positionHint})` : iconicMotif.object;
      ...
      if (storedText.length === directives.length) {
        ...
        const cachedQuality = runQualityGates({
          ...
          humorLevel,
          storySoul,
          concreteAnchors: blueprintV8?.concrete_anchors,
          endingPattern: blueprintV8?.ending_pattern,
          refrainLine: blueprintV8?.refrain_line,
          antagonistName: blueprintV8?.antagonist_dna?.name,
          iconicMotif: blueprintV8?.iconic_motif
        });
        ...
        for (let candidateIdx = 0;candidateIdx < targetCandidateCount; candidateIdx += 1) {
          ...
          let candidateQuality = writeResult.qualityReport ?? toQualitySummary(runQualityGates({
          ...
          const candidateIssuesForTightening = candidateQuality?.issues ?? [];
          const tighteningTargets = limitChapterSet(pickChaptersNeedingTightening(candidateIssuesForTightening), pipelineConfig.maxSentenceTighteningChapters);
          if (ageMaxForTightening <= 8 && tighteningTargets.size > 0) {
            try {
              ...
              if (tightening.changed) {
                ...
                candidateQuality = toQualitySummary(runQualityGates({
                  draft: candidateDraft,
                  directives,
                  cast: castSet,
                  language: normalized.language,
              ...
              console.warn("[orchestrator] sentence-tightening pass failed, continuing with original draft:", tighteningErr?.message || tighteningErr);
            }
          }
          let candidateCritic;
          ...
          const preciseLocalRescue = hasLocalPatchTask && patchTasks.length <= 3 && patchTasks.every((task) => task.chapter > 0 && task.priority <= 2);
          const criticalLocalRepair = qualityPatchTasks.some((task) => task.priority === 1);
          const nearReleaseBand = surgeryEligibleVerdict || preciseLocalRescue || criticalLocalRepair || candidateCritic.overallScore >= Math.max(6, criticWarnFloor - 0.5);
          const rescueableQualityBand = qualityErrors <= 6 || preciseLocalRescue || criticalLocalRepair || candidateCritic.overallScore >= Math.max(6.3, criticWarnFloor - 0.2);
          const candidateSurgeryEdits = maxSelectiveSurgeryEdits;
          const ageFitLow = (candidateCritic.rubricScores?.age_appropriateness?.score ?? 10) < 6 || (candidateCritic.rubricScores?.readability?.score ?? 10) < 6;
          const hardRewriteChapters = new Set;
          if (ageFitLow) {
            const candidateIssues = candidateQuality?.issues;
            if (Array.isArray(candidateIssues)) {
              for (const issue of candidateIssues) {
                if (issue.severity === "ERROR" && (issue.gate === "AGE_FIT_SENTENCE_LENGTH" || issue.gate === "READABILITY_COMPLEXITY" || issue.gate === "LAST_SENTENCE_PIPELINE_ARTIFACT") && issue.chapter > 0) {
            ...
            if (hardRewriteChapters.size === 0) {
              for (const task of patchTasks) {
                if (task.chapter > 0)
                  hardRewriteChapters.add(task.chapter);
              }
            }
          }
          if (surgeryEnabled && (patchTasks.length > 0 || hardRewriteChapters.size > 0) && (hasLocalPatchTask || hardRewriteChapters.size > 0) && nearReleaseBand && rescueableQualityBand && (!candidateCritic.releaseReady || qualityErrors > 0)) {
            const preSurgeryDraft = candidateDraft;
            const surgery = pipelineConfig.wholeStoryEditMode ? await applyWholeStoryEdit({
              storyId: normalized.storyId,
              normalizedRequest: normalized,
              cast: castSet,
              dna: blueprint.dna,
            ...
            if (surgery.changed) {
              ...
              const postSurgeryQuality = toQualitySummary(runQualityGates({
                draft: candidateDraft,
                directives,
                cast: castSet,
                language: normalized.language,
          ...
          if (adaptiveSecondCandidate && candidateIdx === 0) {
            ...
            const firstCandidateStrong = candidateCritic.releaseReady && candidateCritic.overallScore >= Math.max(HONEST_FALLBACK_FLOOR, criticMinScore) && candidateErrors === 0;
            if (firstCandidateStrong) {
              break;
            }
            const criticalIssueCount = countCriticalSelectionIssues(candidateQuality);
            const firstCandidateRetryWorthIt = candidateCritic.overallScore >= 5.8 && candidateCritic.overallScore < Math.max(HONEST_FALLBACK_FLOOR, criticMinScore) && candidateErrors <= 10 || criticalIssueCount > 0 && candidateErrors <= 16 && candidateCritic.overallScore >= 4.2;
            if (!firstCandidateRetryWorthIt) {
              break;
            }
            console.log(`[orchestrator] QW5 honest-fallback: first candidate score=${candidateCritic.overallScore.toFixed(2)} errors=${candidateErrors} \u2192 spawning second candidate.`);
          }
        }
        ...
        await logPhase("phase6-story", { storyId: normalized.storyId, title: storyDraft.title }, {
          ...
          issues: qualityReport?.issues?.map((i) => ({ gate: i.gate, code: i.code, chapter: i.chapter, message: i.message, severity: i.severity })),
      ...
      const strictQualityGatesRaw = normalized.rawConfig?.strictQualityGates;
      const ageMaxForStrict = normalized.ageMax ?? 99;
      const strictQualityGates = typeof strictQualityGatesRaw === "boolean" ? strictQualityGatesRaw : ageMaxForStrict <= 8;
      const strictReleaseGateModeRaw = normalized.rawConfig?.strictReleaseGateMode ?? pipelineConfig.strictReleaseGateMode;
      const strictReleaseGateMode = strictReleaseGateModeRaw === "block" ? "block" : "warn";
      if (qualityReport?.issues?.some((i) => i.code === "MISSING_EXPLICIT_STAKES" || i.code === "CH1_ABSTRACT_STAKES_BEFORE_CONTEXT" || i.code === "CH1_MISSION_UNCLEAR" || i.code === "CH1_GENERIC_CLUE_ENGINE" || i.code === "CH1_ARTIFACT_RULE_UNCLEAR")) {
        ...
        if (repairedStakes) {
          qualityReport = toQualitySummary(runQualityGates({
            draft: storyDraft,
            directives,
            cast: castSet,
            language: normalized.language,
          ...
          await logPhase("phase6-story-local-repair", { storyId: normalized.storyId }, {
            ...
            repaired: true,
            remainingIssues: qualityReport?.issues?.map((i) => i.code)
          });
      ...
      const storyErrors = [...qualityReport?.issues?.filter((i) => i.severity === "ERROR") ?? []];
      const criticActive = releaseEnabled && criticReport && !isCriticSkipped(criticReport);
      const criticHardBlock = criticActive && (criticReport.verdict === "reject" || criticReport.verdict === "revision_needed" && criticReport.overallScore < criticWarnFloor);
      if (criticHardBlock) {
        storyErrors.push({
      ...
      } else if (releaseEnabled && strictQualityGates && criticReport && !isCriticSkipped(criticReport) && (criticReport.verdict === "revision_needed" || criticReport.verdict === "reject")) {
        storyErrors.push({
          gate: "SEMANTIC_CRITIC",
          chapter: 0,
      ...
      if (releaseEnabled && strictQualityGates && criticReport && !isCriticSkipped(criticReport) && criticReport.overallScore < criticMinScore) {
        storyErrors.push({
          gate: "SEMANTIC_CRITIC",
          chapter: 0,
      ...
      const blockingErrors = storyErrors.filter((i) => blockingCodes.has(i.code));
      const strictReleaseErrors = strictQualityGates ? storyErrors.filter((i) => strictReleaseCodes.has(i.code)) : [];
      ...
      const storyGate = {
        ...
        issues: storyErrors.map((issue) => ({ ...issue }))
      };
      ...
      if (strictReleaseErrors.length > 0 && hasContent) {
        ...
        await saveValidationReport(normalized.storyId, validationReport);
        await logPhase(releaseQualityBlocks ? "phase6-story-strict-quality-blocked" : "phase6-story-strict-quality-warning", { storyId: normalized.storyId }, {
          strictQualityGates,
          strictReleaseGateMode,
          releaseCodes: strictReleaseErrors.map((issue) => issue.code),
          safetyBlockingCodes: blockingErrors.map((issue) => issue.code),
          reason: releaseQualityBlocks ? "generated_story_below_release_bar" : "generated_story_below_release_bar_but_content_available"
        });
      }
      if (blockingErrors.length > 0 || !hasContent) {
        ...
        await saveValidationReport(normalized.storyId, validationReport);
        throw new Error(`Story quality gates failed: ${(blockingErrors.length > 0 ? blockingErrors : storyErrors).map((i) => i.code).join(", ")}`);
      }
      if (storyErrors.length > 0) {
        console.warn(`[pipeline] Story accepted with ${storyErrors.length} non-critical quality issues (strictQualityGates=${strictQualityGates}): ${storyErrors.map((i) => i.code).join(", ")}`);
      }
      const phase65Start = Date.now();
      let aiSceneDescriptions = [];
      const aiScenePromptOverride = normalized.rawConfig?.aiScenePromptEnabled ?? normalized.rawConfig?.useAiScenePrompts;
      const aiScenePromptEnabled = typeof aiScenePromptOverride === "boolean" ? aiScenePromptOverride : Boolean(pipelineConfig.aiScenePromptEnabled);
      try {
        ...
        await logPhase("phase6.5-scene-prompts", { storyId: normalized.storyId }, {
          durationMs: Date.now() - phase65Start,
          error: String(sceneGenError?.message || sceneGenError),
          fallbackToTemplate: storyDraft.chapters.length
        });
      }
      const validAiDescriptions = aiSceneDescriptions.filter(Boolean);
      ...
      if (images.length === 0) {
        ...
        try {
          ...
          supplementalError = String(err?.message || err);
        ...
        const supplementalPromptByChapter = new Map(supplementalSpecs.map((spec) => [spec.chapter, spec.finalPromptText || ""]));
      ...
      if (input.enableVisionValidation) {
        ...
        const retryChapters = Object.keys(vision.retryAdvice || {}).map((key) => Number(key)).filter((chapter) => (vision.retryAdvice?.[chapter] || []).length > 0);
        if (retryChapters.length > 0) {
          const retrySpecs = imageSpecs.filter((spec) => retryChapters.includes(spec.chapter)).map((spec) => ({
            ...spec,
...
function buildBlueprintValidationPayload(result) {
  if (!result)
    return;
  return {
    version: "v8",
...
function buildPass3ValidationPayload(report) {
  if (!report || isCriticSkipped(report))
    return;
  return {
...
function toQualitySummary(report, rewriteAttempts) {
  if (!report) {
    return {
      score: 0,
      passedGates: [],
  ...
  if (typeof report.score === "number" && typeof report.issueCount === "number" && typeof report.errorCount === "number" && typeof report.warningCount === "number") {
    return {
      ...report,
...
function applyExplicitStakesRepair(draft, input) {
  ...
  if (cleanedOriginal.toLowerCase().startsWith(orientation.toLowerCase().slice(0, 80)))
    return false;
...
function buildConcreteChapter1Orientation(input) {
  ...
  const blueprintContract = input.blueprint?.reader_contract || {};
  const soulContract = input.storySoul?.readerContract || {};
  const childNames = input.cast ? input.cast.avatars.map((a) => a.displayName).filter(Boolean) : [];
  ...
  const artifactName = input.cast?.artifact?.name || (isGerman ? "das besondere Fundst\xFCck" : "the special object");
  const normalWorld = meaningfulText(blueprintContract.normal_world, soulContract.normalWorld, isGerman ? `${lead} und ${companion} standen an einem vertrauten Ort, bevor das Problem begann.` : `${lead} and ${companion} stood in a familiar place before the problem began.`);
  const mission = sanitizeConcreteMission(meaningfulText(blueprintContract.mission_in_child_words, soulContract.missionInChildWords, isGerman ? `${lead} und ${companion} mussten ${artifactName} rechtzeitig an den richtigen Platz bringen.` : `${lead} and ${companion} had to bring ${artifactName} to the right place in time.`), { isGerman, lead, companion, artifactName });
  const why = meaningfulText(blueprintContract.why_it_matters, soulContract.whyItMattersNow, isGerman ? "Sonst blieb heute ein wichtiger Platz leer und jemand wurde entt\xE4uscht." : "Otherwise an important place would stay empty today and someone would be disappointed.");
  const rule = meaningfulText(blueprintContract.special_rule, soulContract.magicOrArtifactRule, input.cast?.artifact?.storyUseRule ? `${artifactName} ${input.cast.artifact.storyUseRule}; ${isGerman ? "entscheiden mussten die Kinder selbst." : "the children still had to decide themselves."}` : isGerman ? `${artifactName} zeigte nur, was anders war; l\xF6sen mussten ${lead} und ${companion} es selbst.` : `${artifactName} only showed what was different; ${lead} and ${companion} had to solve it themselves.`);
  return [normalWorld, mission, why, rule].map(ensureSentence).filter(Boolean).join(" ");
}
...
function sanitizeConcreteMission(mission, input) {
  const lower = mission.toLowerCase();
  const clueOnly = /\b(naechsten|n\u00E4chsten|ersten|letzten)?\s*(hinweis|spur|zeichen|weg)\s*(finden|folgen|erreichen|lesen|suchen)\b/.test(lower) || /\b(next|first|last)?\s*(clue|trail|sign|path)\s*(find|follow|reach|read|search)\b/.test(lower);
  ...
  if (!clueOnly || concreteTask)
    return mission;
  return input.isGerman ? `${input.lead} und ${input.companion} mussten ${input.artifactName} rechtzeitig an den richtigen Platz bringen.` : `${input.lead} and ${input.companion} had to bring ${input.artifactName} to the right place in time.`;
}
...
function removeGenericAbstractOpening(text, language) {
  const isGerman = language === "de";
  const pattern = isGerman ? /^\s*Wenn die Kinder den wichtigen Hinweis verlieren,\s*bleibt der Weg zum Ziel verschlossen\.\s*/i : /^\s*If the children do not protect the clue,\s*they lose the path to the goal\.\s*/i;
  return String(text || "").replace(pattern, "");
}
function shouldSkipSemanticCritic(quality) {
  if (!quality)
    return false;
  const issues = Array.isArray(quality.issues) ? quality.issues : [];
  const hasPlaceholder = issues.some((issue) => issue?.code === "CHAPTER_PLACEHOLDER");
  return hasPlaceholder;
}
function buildSkippedCriticReport(model) {
  return {
    model,
    overallScore: 0,
    dimensionScores: {
...
function mergePatchTasks(criticTasks, qualityTasks) {
  const seen = new Set;
  const merged = [];
  for (const task of [...qualityTasks, ...criticTasks]) {
    if (!task || task.chapter <= 0)
      continue;
  ...
  return merged.sort((a, b) => a.priority - b.priority || a.chapter - b.chapter).slice(0, 5);
}
function derivePatchTasksFromQualityGates(input) {
  ...
  const artifactRule = input.cast.artifact?.storyUseRule?.trim() || input.storySoul?.readerContract?.magicOrArtifactRule || "it points at the truth but never solves the choice for the children";
  const finalChapter = Math.max(1, input.chapterCount || input.directives.length || 5);
  const antagonistName = String(input.blueprint?.antagonist_dna?.name || findAntagonistName(input.cast) || "").trim();
  const addTask = (chapter, priority, objective, instruction) => {
  ...
  const addArtifactArcTasks = () => {
    addTask(1, 1, "Put artifact rule into Chapter 1", `Name ${artifactName} by paragraph 2, explain one child-readable rule (${artifactRule}), and tie it to the visible mission.`);
    addTask(findArtifactChapter(input.directives, "discovery") || 2, 1, "Stage artifact discovery", `Show ${artifactName} being discovered or chosen on page. Let a child touch/use it once; no clue-only summary.`);
    addTask(findArtifactChapter(input.directives, "success") || Math.min(4, finalChapter), 1, "Stage artifact payoff", `Use ${artifactName} in the scene to reveal/confirm truth, but the child must make the brave decision.`);
  };
  for (const issue of issues) {
    ...
    if (code === "ARTIFACT_UNDERUSED" || code === "ARTIFACT_UNDERMENTIONED" || code === "MISSING_DISCOVERY" || code === "MISSING_SUCCESS") {
      addArtifactArcTasks();
      continue;
    }
    if (code === "ARTIFACT_PREMATURE_ACTIVE") {
      addTask(chapter > 0 ? chapter : findArtifactChapter(input.directives, "discovery") || 2, 1, "Repair artifact continuity", `Before ${artifactName} is physically discovered or recovered, it may not glow, warm, jump, protect, answer, or solve anything. Replace premature magic with visible clue-work, then stage the proper discovery/payoff in order.`);
      continue;
    }
    if (chapter > 0 && (gate === "DIALOGUE_QUOTE" || code.includes("DIALOGUE_RATIO"))) {
      addTask(chapter, severity === "ERROR" ? 1 : 2, "Add purposeful dialogue", "Add 2-3 short dialogue lines with friction, warmth, or humor. Each line must reveal character, not explain plot.");
      continue;
    }
    if (chapter > 0 && code === "VOICE_MARKERS_TOO_FEW") {
      addTask(chapter, 2, "Sharpen character voice", "Give the named child one recognizable body tell, one favorite-word rhythm, and one distinct short line.");
      continue;
    }
    if (chapter === 1 && (code.startsWith("CH1_") || gate === "READER_CONTRACT")) {
      addTask(1, 1, "Fix Chapter 1 orientation", "Open with who, where, mission, why now, and one concrete consequence before clues, chase, or abstract stakes.");
      continue;
    }
    if (code === "ENDING_PATTERN_NOT_REALIZED" || code === "ENDING_PAYOFF_ABSTRACT" || code === "ENDING_WARMTH_MISSING" || code === "GOAL_THREAD_WEAK_ENDING") {
      addTask(finalChapter, 1, "Make ending concrete", "End with a visible changed action, a small price paid, a Chapter 1 callback, and a warm final image.");
      continue;
    }
    if (code === "ANTAGONIST_TOO_FEW_APPEARANCES" || code === "ANTAGONIST_NO_SHOWDOWN") {
      addTask(Math.max(1, finalChapter - 1), 1, "Put antagonist on stage", antagonistName ? `Put ${antagonistName} or its visible damage on stage and force a child choice; avoid mere mention.` : "Put the opposing force or its visible damage on stage and force a child choice; avoid mere mention.");
    }
  }
  ...
  const seen = new Set;
  return tasks.filter((task) => task.chapter > 0).sort((a, b) => a.priority - b.priority || a.chapter - b.chapter).filter((task) => {
...
function findArtifactChapter(directives, kind) {
  const pattern = kind === "discovery" ? /\b(discover|find|found|first|entdeck|erst|bekomm|erhalt)\b/i : /\b(use|uses|success|solve|reveal|setz|nutz|benutz|loes|l\u00F6s|rett|bring)\b/i;
  const match = directives.find((directive) => pattern.test(String(directive.artifactUsage || "")));
  return match?.chapter;
}
function findAntagonistName(cast) {
  const antagonist = cast.poolCharacters.find((character) => {
    ...
    return values.some((value) => value.includes("antagonist") || value.includes("villain") || value.includes("boes") || value.includes("b\xF6se"));
  });
  return antagonist?.displayName || "";
}
function scoreReleaseCandidate(quality, critic, releaseEnabled) {
  ...
  const effectiveQualityScore = qualitySignalCollapsed ? Math.min(criticScore, 4.8) : qualitySignalWeak ? Math.max(qualityScore, criticScore - 2) : qualityScore;
  const blend = qualitySignalWeak ? effectiveQualityScore * 0.42 + criticScore * 0.58 : effectiveQualityScore * 0.4 + criticScore * 0.6;
  const errorPenaltyWeight = qualitySignalWeak ? 1.05 : 1.2;
  const warningPenaltyCap = qualitySignalWeak ? 1.5 : 1.8;
  const penalties = errorCount * errorPenaltyWeight + Math.min(warningPenaltyCap, warningCount * 0.04) + criticalIssueCount * (qualitySignalWeak ? 0.34 : 0.3);
...
function isCriticSkipped(critic) {
  if (!critic)
    return false;
...
function countCriticalSelectionIssues(quality) {
  const issues = Array.isArray(quality?.issues) ? quality.issues : [];
  ...
  for (const issue of issues) {
    if (!issue || issue.severity !== "ERROR")
      continue;
    if (criticalCodes.has(String(issue.code || "")))
      count += 1;
  }
  if (failedGates.has("LENGTH_PACING"))
    count += 2;
  if (failedGates.has("DIALOGUE_QUOTE"))
    count += 2;
  if (failedGates.has("STAKES_LOWPOINT"))
    count += 2;
  if (failedGates.has("IMAGERY_BALANCE"))
    count += 1;
  if (failedGates.has("CAST_LOCK"))
    count += 2;
  if (failedGates.has("CHILD_MISTAKE_ARC"))
    count += 2;
  if (failedGates.has("ENDING_PAYOFF"))
    count += 1;
  return count;
}
...
async function selectBestFairyTale(input) {
  ...
  const recentRows = await storyDB.queryAll`
    SELECT si.tale_id
    ...
    LIMIT 8
  `;
  const recentIds = recentRows.map((row) => row.tale_id).filter((value) => Boolean(value));
  ...
  for (const candidate of pool) {
    ...
    let score = 0;
    if (rangesOverlap(ageMin, ageMax, input.ageMin, input.ageMax))
      score += 2.2;
    else
      score -= 4;
    if (ageMin <= input.ageMin && ageMax >= input.ageMax)
      score += 1.2;
...
function parseThemeTags(tagsJson) {
  ...
  if (typeof value === "string") {
    try {
      ...
      if (Array.isArray(parsed))
        return parsed.map((entry) => String(entry).toLowerCase()).filter(Boolean);
    } catch {
...
function extractProtagonistConstraints(rolesJson) {
  const roles = parseJsonArray(rolesJson);
  return roles.filter((role) => String(role?.roleType || "").toUpperCase() === "PROTAGONIST").flatMap((role) => parseStringArray(role?.constraints));
}
function scoreYoungReaderTaleFit(candidate) {
  ...
  let score = 0;
  if (constraints.some((value) => value.includes("age=child")))
    score += 2.4;
  if (constraints.some((value) => value.includes("species=animal")))
    score += 0.6;
  if (constraints.some((value) => value.includes("age=young_adult") || value.includes("age=adult")))
    score -= 2.2;
...
function buildTaleSelectionCorpus(candidate) {
  return [
    candidate.title,
    candidate.tone || "",
    candidate.coreConflict,
...
function scoreCommercialSeriesFit(candidate, input, corpusOverride) {
  ...
  if (humorLevel >= 2 || wantsWittyTone) {
    if (humorFriendlyPattern.test(corpus))
      score += 1.9;
    if (hasMelancholyToneMarkers(corpus))
      score -= 3.6;
  ...
  if (suspenseLevel >= 2 || input.hasTwist) {
    if (mysteryPattern.test(corpus))
      score += 2.1;
    if (slowPoeticPattern.test(corpus))
      score -= 1.2;
  } else if (suspenseLevel >= 1 && mysteryPattern.test(corpus)) {
    score += 0.9;
  }
  if (wantsFastPace) {
    if (fastPattern.test(corpus))
      score += 1.4;
    if (hasMelancholyToneMarkers(corpus) || slowPoeticPattern.test(corpus))
      score -= 1.8;
  }
  if (avatarCount >= 2) {
    if (teamPattern.test(corpus))
      score += 1.3;
    if (lonelyPattern.test(corpus))
      score -= 1.8;
  }
...
function tokenizeTone(tone) {
  if (!tone)
    return [];
  return tone.toLowerCase().split(/[^a-zA-Z\u00E4\u00F6\u00FC\u00C4\u00D6\u00DC\u00DF]+/).map((token) => token.trim()).filter((token) => token.length >= 3);
}
function hasDarkToneMarkers(text) {
  if (!text)
    return false;
  return /dark|dunkel|tragic|tragisch|horror|grausam|death|tod|bitter|dread|schrecken|haunted/i.test(text);
}
function hasMelancholyToneMarkers(text) {
  if (!text)
    return false;
...
function sanitizeMemoryTitleForPrompt(value) {
  ...
  if (lastWord.length <= 1)
    return "";
  if (/\b(?:warum|wieso|weshalb|why)\s+[a-z]$/i.test(normalized))
    return "";
  return normalized.length <= 120 ? normalized : `${normalized.slice(0, 117).trimEnd()}...`;
}
function sanitizeMemoryExperienceForPrompt(value) {
  const normalized = String(value || "").replace(/["']/g, "").replace(/\s+/g, " ").trim();
  if (!normalized)
    return "";
  return normalized.length <= 160 ? normalized : `${normalized.slice(0, 157).trimEnd()}...`;
}
function withPipelineStoryCostControls(normalized, pipelineConfig) {
  const raw = normalized.rawConfig;
  ...
  const providerAwareMaxExpandCalls = openRouterSelected ? Math.max(1, Number(pipelineConfig.maxExpandCalls || 0)) : pipelineConfig.maxExpandCalls;
  const providerAwareMaxStoryTokens = openRouterSelected ? Math.max(Number(pipelineConfig.maxStoryTokens || 0), 22000) : pipelineConfig.maxStoryTokens;
  return {
    ...normalized,
    rawConfig: {
      ...raw,
...
function limitChapterSet(chapters, maxChapters) {
  const max = Math.max(0, Math.floor(Number(maxChapters) || 0));
  if (max === 0 || chapters.size === 0)
    return new Set;
  return new Set([...chapters].sort((a, b) => a - b).slice(0, max));
}
async function logPhase(source, request, response) {
...
async function fetchArtifactMeta(artifactId) {
  if (!artifactId)
    return null;
  try {
