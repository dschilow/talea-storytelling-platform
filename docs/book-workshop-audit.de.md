...
**Flux bleibt `runware:400@4`.** Auf deinen ausdrücklichen Wunsch bleibt die Sprite-Collage als einzelne Referenz erhalten. Die aktuelle Bilderbuch-Pipeline und die neue Buchwerkstatt verwenden jetzt einen gemeinsamen rahmenlosen Renderer. Die Buchwerkstatt kombiniert die tatsächlich sichtbaren Figuren und gegebenenfalls das Artefakt in einem Bild; auch vier Figuren plus Artefakt benötigen nur einen Referenzeingang. Die Zuordnung erfolgt von links nach rechts, ohne hinzugefügte Farbrahmen oder Beschriftungen. Jeder Auftrag beginnt mit dem Moment der freigegebenen Seite. Das erste Seitenbild kann als Cover dienen. Zwei Aufträge laufen gleichzeitig; ein fehlgeschlagener Versuch wird nicht automatisch mit neuer Kennung erneut bezahlt. Runware unterstützt Referenzen als Data URI, wodurch die lokal zusammengesetzte Collage direkt übertragen werden kann. [Runware-Dokumentation](https://runware.ai/docs/models/bfl-flux-2-klein-4b).

Das entfernt die künstlichen Rahmen aus der Collage-Eingabe, garantiert aber noch keine korrekten Bilder. Vor der Veröffentlichung der neuen Route fehlen echte Flux-Vergleichsläufe und eine visuelle Abnahme auf Figurenidentität, Rahmen, falsche Requisiten und Textwidersprüche. Für das Zielsystem würde ich eine günstige Bildprüfung mit klar begrenzter Wiederholung vorsehen. Außerdem braucht der PDF-Export eine bewusste Buchgestaltung: Im geprüften Dokument stehen kleine quadratische Bilder über relativ großen Textblöcken mit viel ungenutzter A4-Fläche. Die sichtbaren Umlaute sind korrekt; ungewöhnliche Zeichen bei der Textextraktion sind kein belegter Fehler im gerenderten PDF.
...
Im Story-Wizard gibt es jetzt eine eigenständige Auswahl **Buchwerkstatt (Neu · Test)**. Sie sendet `storybookMode: true` und `storybookEngine: "book-workshop-v1"`; Bilderbuch sendet ausdrücklich `storybookEngine: "storybook-v1"`. Die Wizard-Auswahl hat Vorrang vor `TALEA_STORYBOOK_ENGINE`. Diese Servervariable bleibt nur als Vorgabe für ältere Clients ohne explizite Engine erhalten. Für die neue Auswahl muss sie nicht gesetzt werden. Frontend und Backend müssen gemeinsam deployed werden; hier wurde nichts deployed. Der Kandidat routet Textaufrufe derzeit über OpenRouter, auch wenn eine native Wizard-Modellbezeichnung übersetzt wird. Native Anbietertransporte und jede einzelne Modellkennung sind damit noch nicht Ende zu Ende geprüft. Unbekannte Modelle brauchen vor dem ersten Textaufruf einen verfügbaren Preis; es gibt keinen stillen Modellwechsel.
...
Bestanden nach dem Collage-Fix: **37 Tests des neuen Kerns, 43 Bilderbuchtests und 7 Tests des gemeinsamen Referenz-Renderers** (87 zusammen). Die 25 Prüfungen der vorhandenen Bildpipeline bestanden bereits zuvor. Geprüft werden unter anderem fehlerhafte und abgeschnittene Antworten, bezahlte Fehlversuche, fehlende Prüfungen, Widerspruchssperren, neue Prüfungen nach Überarbeitung, Avatarbeiträge, Elternvorgaben und Bildkosten. Die neuen Referenztests prüfen zusätzlich Rahmenpixel, vollständige Figuren ohne Beschnitt, Identitätsreihenfolge, Caching und genau einen Referenzeingang auch bei vier Figuren plus Artefakt. Ein Vergleich des gesamten Backend-Typechecks mit einer virtuellen unveränderten HEAD-Fassung ergibt dieselben 52 bestehenden TypeScript-Diagnosen und keine hinzugekommenen. Das Backend hat damit weiterhin keinen insgesamt fehlerfreien Typecheck.

Der Referenz-Renderer wurde außerdem mit den echten Exportbildern von Bäcker Wilhelm und Hexe Griselda ausgeführt. Die lokal gespeicherte Collage wurde visuell geprüft: keine hinzugefügten Rahmen, beide Quellbilder vollständig erhalten. Der anschließende einzelne Flux-Test mit `runware:400@4` wurde am 7. September 2026 mit HTTP 401 abgewiesen. Es entstand kein prüfbares Ausgabebild. Die visuelle Abnahme der erzeugten Szene bleibt daher offen; auch die Bildkosten dieses Versuchs sind nicht vom Anbieter bestätigt. Lokale Diagnoseartefakte liegen unter `Logs/frame-fix/` und enthalten keinen API-Schlüssel.
