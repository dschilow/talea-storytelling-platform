# 🚀 Talea Railway Deployment Guide
...
3. **API Keys** → **Secret keys** → Copy **Secret key**
...
1. Go to [Clerk Dashboard](https://dashboard.clerk.com)
2. Select your application
3. **API Keys** → **Quick Copy** → **Next.js** → Copy the publishable key
...
- ✅ `backend/infra.config.railway.json` - Encore infrastructure config
- ✅ `backend/start.sh` - Backend startup script
- ✅ `nginx.conf` - Frontend nginx configuration
- ✅ `docker-entrypoint.sh` - Frontend entrypoint script
- ✅ `backend/health/health.ts` - Health check endpoint

### Updated Files:
- ✅ `backend/encore.app` - Added CORS configuration
...
1. **Monitor Logs:** Railway → Each service → "Logs" tab
2. **Set up Custom Domains:** Railway → Service → Settings → Domains
3. **Production Clerk Keys:** Replace dev keys with production keys
4. **Scale Services:** Railway → Service → Settings → Resources
